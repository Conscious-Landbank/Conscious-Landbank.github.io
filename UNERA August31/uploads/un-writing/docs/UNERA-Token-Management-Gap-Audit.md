# Huma token management: feature gap audit and improvement plan

> **What this is.** A page-by-page audit of the current Huma consumer pages against the requirements in [`docs/UNERA-Token-Management.md`](./UNERA-Token-Management.md) (Confluence v17). For each feature it lists: what the doc requires, what the page currently has, the **gaps**, and **detailed, design-system-consistent improvements** to close them.
>
> **Source of truth for design.** Every improvement below references the **Huma Design System V2** in this project, its tokens, its components and its pattern cards, so anything built stays 100% consistent with the uploaded files. Design-system anchors are cited inline as `-> DS:`.
>
...
>
> **Reference PRDs:** [Huma: Token Management](https://conscious-landbank.atlassian.net/wiki/spaces/~7120206e88bad7379045fb808e0bd72c8daf86/pages/62259420) (v17) for the overall transaction set, and the dedicated [Huma Token Swap](https://conscious-landbank.atlassian.net/wiki/spaces/~7120206e88bad7379045fb808e0bd72c8daf86/pages/56525008) (v3) for Swap mechanics. **§3 reconciles the two where they conflict.**
>
...
- "Who pays gas" must be explicit on every Phase-1 preview (§6 common rules). Send does this well (fee tooltip: "paid to the network, not Huma"); Swap/Buy do not yet.
...
- G2.1 / G2.3 / G2.7: Rebuild the Conversion Summary rows. Use the existing summary-row pattern; final row order:
  ...
  4. Estimated network gas: **"Covered by Huma"** pill (this is the platform's transfer; user pays nothing). -> **DS: Core card** badge; `--fin-up`/`--fin-up-bg` for the "covered" affirmative.
...
- Two directions = Mint & Burn:
  - Mint (stablecoin USDC/USDT -> Huma hCAD): the system locks the stablecoin in reserve and **mints** the equivalent Huma to the user's wallet.
  - Burn (Huma -> stablecoin): the system **burns** the user's Huma and unlocks the equivalent stablecoin from reserve.
...
- Related surfaces (scope-flag, not part of the swap card): a public **Reserve Dashboard** (total stablecoins in reserve, total Huma held, health ratio) and a **Liquidity Provider Portal** (deposit/withdraw, APY, dynamic withdrawal fee). See §3.4.
...
- Price impact (dummy calc): **this should be removed** (no price impact in an Oracle model).
...
- Fee/gas rows in the confirm sheet:
  ...
  - One-line note: **"You pay network gas for both steps. Huma adds no markup."** -> mirrors Send's `fee-info-btn` tooltip language.
...
- Reserve Dashboard (public): total stablecoins in reserve, total Huma held, **health ratio**. *You already have `proof-of-reserve-public.html`*, audit it against this list as a follow-up; it likely covers most of it.
...
- Network fee row with tooltip: *"Network (gas) fee is paid to the blockchain network, not Huma…"* + **Total**. -> AC-SEND-04/05, §6 (who pays gas), **exemplary**.
...
|----|-------|-----|-----------|
| **X1** | **Token naming drift** (hCAD vs hUSD) | Choose the canonical symbol per surface and apply to every row, helper, and receipt. Doc + wallet use **hUSD**; if a surface really is CAD-denominated, label it **hCAD** consistently, never mix within one flow. | `readme.md` content rules |
| **X2** | **"Who pays gas" inconsistent** | Every Phase-1 preview states gas payer explicitly. Send is the template ("paid to the network, not Huma"). Apply the same `fee-info-btn` tooltip to Swap/Trade/Buy. | **Core card** `fee-info-btn` |
