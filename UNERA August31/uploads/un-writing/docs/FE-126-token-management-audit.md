...
Sources: Confluence **Huma – Token Management** (62259420), **Transaction History** (68583557), Jira **FE-126** + **FE-127**, and the **20 Jun huddle recording** (transcript `uploads/Archive/…-transcript.md` + 69 frames).
...
- D · Activity sub-transactions (`wallet-enhanced.html`): the transaction-detail modal now shows a **Sub-transactions** group: Buy hUSD -> *Send Fiat -> Receive hUSD*; Swap -> *Approve -> Swap*; Trade -> *Authorize -> Trade*; plain transfers show none.
...
### C · Buy: needs your decision (didn't silently pick; PRD conflict)
...
2. Provider quotes. The video's MetaMask refs show MoonPay/Mercuryo/Banxa because MetaMask aggregates third-party on-ramps. **Huma is its own OTC provider (delivers from inventory)**, so third-party provider-quote UI would contradict §5.1. -> **Confirm we keep the single-provider OTC preview (recommended) and drop the MoonPay/Banxa idea.**
...
## Remaining backlog (prioritized: needs go-ahead)
...
| Flow | Reference Eric demoed | Huma page |
|---|---|---|
