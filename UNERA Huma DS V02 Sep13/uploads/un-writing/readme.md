...
**Huma Design System V2** is the visual and interaction language for **Unera Stablecoin**, a social-purpose fintech platform.
Officially: *Unera Brand Guidelines, Version 1 (March 2026).*

> **One Flow. Many Lives.** *Value, Shared by Design.*

Huma is the institutional identity behind Unera Stablecoin: stable digital infrastructure that connects people and moves value with purpose. One flow of stable digital value supports many lives through **Humanity Centers**: donations, remittances, community well-being. The system is built to make a high-stakes financial action feel calm.

It encodes a mission: reduce cognitive load, signal trust, and keep humanity visible without sacrificing data density.

## Where the pages are (consolidated 22 Aug 2026)

All 42 consumer pages live in **`unera-pages/`**. Open `unera-pages/_all-screens.html` for a visual index. Superseded drafts are in `unera-pages/_archive/`. Provenance and verification: `MERGE-AUDIT.md`.
...
- GitHub: [Conscious-Landbank/Conscious-Landbank.github.io](https://github.com/Conscious-Landbank/Conscious-Landbank.github.io), the full repository (Brand Guide, NewUnera, Mobile App, Roadmap). Explore it for deeper context when building new Huma work.
- Brand fonts: TestFoundersGrotesk OTF family (Light->Bold, normal + italic), supplied by the brand.
- Logos: Huma wordmark lockups (white/black, full + nav), supplied by the brand.

> The brand-style-guide is the **specimen authority**; the consumer pages are the **behavioral authority**.

## Design philosophy

1. Clarity over cleverness. Finance users need scannable hierarchy, not decorative novelty. Type carries meaning before copy does, and so do spacing and color.
...
3. Humanity in the architecture. Impact stats, center discovery, donation flows, and warm earth/cloud tints remind users value has a destination beyond the ledger.
...
- Person. Speak to the user as **you** ("Track your impact", "Choose where your value lands"). The institution is **Huma / we** in narrative copy ("Huma builds stable digital infrastructure…").
- Casing. Sentence case for body, labels, and buttons ("Add Tokens", "Buy Stablecoins"). **UPPERCASE only** for nav links and small eyebrow labels (`DASHBOARD`, `WALLET`, `TRANSACT`, `CENTERS`, section eyebrows). Page titles are Title/sentence case in solid Deep Blue.
...
- Emoji. Avoid them, with one sanctioned exception: the MetaMask fox glyph in the wallet badge. No decorative emoji, no emoji cards.
- Vibe: trustworthy infrastructure with a human pulse. Mission-forward but never preachy.

## VISUAL FOUNDATIONS

**Color.** Anchored on **Deep Blue `#173d47`**, a teal-navy that reads institutional and grounded, well away from fintech emerald or sky-blue clichés. It carries nav, primary buttons, page titles, and stepper emphasis.
...
**Type.** One grotesk family for display and body: **TestFoundersGrotesk** (brand intent: Oakes Grotesk; Oakes is the named fallback). **Weight and size create hierarchy, not font switching.** Page titles are **solid Deep Blue, with no gradient text on product screens**. KPI figures use `--font-stat-size` (2.5rem) with tabular numerals. Body ~1.6 line-height, antialiased. CJK -> Noto Sans SC; Arabic -> Mada (Google Fonts, loaded when needed).
...
**Cards.** `border-radius: 1.25rem` (20px), 1–2px subtle tinted border, soft resting shadow (`--shadow-card`). On hover: border -> Deep Blue, shadow deepens (`--shadow-hover`), and the card **lifts** (`translateY(-4px)`, impact/center cards `-6px`). Impact & center cards add a Deep Blue top rule that wipes in on hover.
...
**Transparency & blur.** Sparingly: the center "Verified" badge uses `backdrop-filter: blur(10px)` over imagery; nav menus and modals use solid surfaces with soft shadows, not glass.

**Imagery vibe.** Warm, human, documentary photography of communities and Humanity Centers (full-bleed inside rounded card tops, subtle zoom on hover). Not cold stock fintech, not b&w, not heavy grain.
...
- System: Material Symbols (Outlined), used as **inline SVG only**, `fill="currentColor"`. No icon fonts, no PNG icons, no emoji (except the sanctioned MetaMask fox in the wallet badge).
...
- Logos: `assets/logos/` holds `huma-white-text.svg` / `huma-black-text.svg` (primary lockups) and `…-nav.svg` (tighter nav lockups), mirrored in `unera-pages/NewLogo/` as `Huma White Text Nav.svg` etc. for the pages that load them by that path. White lockup on Deep Blue; black on light. Never recolor the wordmark. The lockup is the shared arc device plus a **Huma** wordmark set in TestFoundersGrotesk Regular, drawn on the same `0 0 361.1 113.22` viewBox as the retired Unera lockup so nav sizing is unchanged (nav height stays 24px). **Interim asset** pending real brand work; see `CHANGES-FE-207.md`. The Unera lockups (`unera-*.svg`) stay in the folder: they are the **stablecoin/issuer** brand and are still used by `brand-style-guide.html`.
...
**Principal designer's reminders:** brand-vs-money color is the #1 implementation mistake, so teach it early. Nav is infrastructure, not a per-page experiment. Receipts need density (proof, not just celebration). Flow terminal states are **asymmetric**: success and failure are different components, not recolored heroes. Humanity Centers are first-class IA (in the primary nav). Wallet connection is **progressive** (CONNECT -> pill -> dropdown). Don't skip the disconnected state.

## Index of what is in this system

**Foundations (root):**
- `styles.css`: the single entry point consumers link (an `@import` manifest).
- `tokens/colors.css` · `tokens/typography.css` · `tokens/surfaces.css` · `tokens/spacing.css`: all CSS custom properties plus `@font-face`.
- `fonts/`: TestFoundersGrotesk OTF family. `assets/logos/`: Huma wordmarks.
...
**UI kits** (`ui_kits/`):
- `consumer-app/`: interactive click-through of the consumer stablecoin app: nav (dual-session wallet pill), Dashboard (impact KPIs + quick actions), Wallet (balance hero + tokens + activity), Humanity Centers (discovery + donate), and the multi-step Send flow (stepper -> review -> success receipt). Entry: `index.html`. Pattern cards: Wallet Patterns, Notifications, Popups & Modals, **Transact Flows** (the four TRANSACT actions, Add Tokens, Send, Exchange and Stake, with shared patterns plus per-action components).
