...
- `wallet-enhanced.html`: the KYC banner moved out of the top of `<main>` into the attention strip (and lost its container-bleeding negative margins there); the four-tile `.quick-actions` grid was **removed** because the hero now owns those CTAs; a new `Impact & Huma Points` strip (Huma Points 1,284 with the earn rates · Donated $5,240 · Lives impacted 1,240+) was added last. **Balances now default to visible** (`balancesHidden = false`) with privacy as an opt-in toggle, because a blurred hero defeats the point of the hierarchy.
