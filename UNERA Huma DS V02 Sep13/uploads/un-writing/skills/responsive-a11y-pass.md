...
## Responsive: check ~360 (small mobile), 768 (tablet) and 1440 (desktop)
...
- Buttons in horizontal flex banners: base `.btn` is often `width:100%`, so reset to `width:auto` inside rows, add `flex-wrap`, and a ≤560px rule that stacks full-width.
...
- Images: decorative photos `alt=""`; never `src=""` (it refetches the page URL and throws a console error, so set `src` from JS or omit the attribute).
