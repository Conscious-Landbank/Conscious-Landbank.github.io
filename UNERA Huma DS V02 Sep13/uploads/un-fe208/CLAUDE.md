...
- **Waiting is part of the product.** A pending transaction is the moment a non-crypto user gets scared. Plain language, an ETA, named stages, reassurance and something to read beat a spinner every time.
...
## 4b. Pending transactions — always the shared Transaction Tracker (FE-207 §B)
...
- States: **running → delayed → done | failed**, plus the in-memory dev pills (Normal / Taking longer / Failed / Done now). Delayed *holds* — the user may leave and come back via the pill.
