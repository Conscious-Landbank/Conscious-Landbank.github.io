...
function feeUSD() { return D.processingFee(method, totalUSD()); }
...
function renderAmountUI() {
  ...
  if (crypto && walletOk) {
    ...
    $('amountHint').textContent = 'Donations between ' + D.fmtUSD(MIN_USD) + ' and ' + D.fmtUSD(MAX_USD) + ' (USD value) per transaction.';
    $('conversionTipText').innerHTML = f.code === 'USD'
      ? 'Your card is charged in USD and the donation is routed to the ' + D.SETTLEMENT.fiatAccountLabel + '. Fiat is never converted to crypto.'
      : 'Your card is charged in ' + f.code + '. The payment rail converts it to USD before routing — we keep your original amount, the rate and the USD value on the record.';
...
function renderReview() {
  ...
  if (method === 'fiat') {
    ...
    orderRows += row('Wallet address', D.shortAddr(D.SETTLEMENT.multisig), { mono: true });
    orderRows += row('Network', D.SETTLEMENT.network + ' <span class="tbd-tag">TBD</span><span class="summary-row-note">Approved network list is still open (OQ-03)</span>');
  }
...
function trackerConfig(donationId, c, refLabel) {
  ...
  var stages = crypto
    ? [
        { title: 'Waiting for your signature', sub: 'Approve the request in your wallet. Nothing is sent until you do.' },
        { title: 'Submitted to the network', sub: 'Your transaction is broadcast. Nothing else is needed from you.' },
        { title: 'Awaiting confirmation', sub: 'The blockchain is writing your donation into a block.', conf: true }
      ].concat(converts ? [
        { title: 'Converting to ' + settleAs, sub: 'Our Crypto Swap Worker is converting your ' + token + ' to ' + settleAs + ' at the quoted route. This runs on its own.' }
      ] : []).concat([
        { title: 'Delivered to the ' + D.SETTLEMENT.multisigLabel, sub: 'The funds arrive and your digital receipt is issued.' }
      ])
    : [
        ...
        { title: 'Routed to the settlement account', sub: 'We send the donation on to the account configured for this Humanity Center.' },
  ...
  return {
    ...
    headlineDelayed: outcome === 'timeout' ? D.ERR.timeout : 'Taking a little longer than usual — that’s normal when the network is busy. Nothing to do.',
    ...
    facts: [
      'Donations start at just $1 — small, regular giving adds up fastest.',
      'Processing fees are added on top of your donation, never taken out of it.',
      'Every donation is tied to a transaction id, so impact is verifiable, not just reported.',
...
function terminal(kind, donationId) {
  ...
  if (kind === 'success') {
    ...
    html = '<div class="amount-section terminal-state flow-card-animated">'
      ...
      + detailRow(multi ? 'Humanity Centers' : 'Humanity Center', centreLabel)
      + detailRow('Source', method === 'fiat' ? 'Fiat — card rail' : 'Crypto — ' + token)
