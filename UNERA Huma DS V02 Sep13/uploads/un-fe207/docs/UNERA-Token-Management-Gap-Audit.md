# Huma — Token Management: Feature Gap Audit & Improvement Plan
...
>
> **Source of truth for design.** Every improvement below references the **Huma Design System V2** in this project — its tokens, components, and pattern cards — so anything built stays 100% consistent with the uploaded files. Design-system anchors are cited inline as `→ DS:`.
>
...
- **Two directions = Mint & Burn:**
  - **Mint** — stablecoin (USDC/USDT) → Huma (hCAD): system locks the stablecoin in reserve and **mints** the equivalent Huma to the user's wallet.
  - **Burn** — Huma → stablecoin: system **burns** the user's Huma and unlocks the equivalent stablecoin from reserve.
