...
(function () {
    ...
    function illustration(kind, labels) {
        ...
        return '' +
            ...
            '<line class="txt-track txt-seg txt-seg-0" x1="59" y1="40" x2="133" y2="40"/>' +
            '<line class="txt-track txt-seg txt-seg-1" x1="167" y1="40" x2="241" y2="40"/>' +
            /* Kevin, 29 Aug (28 Aug thread): animated media, no mascots - value dots travel the route while the transaction runs */
            '<circle class="txt-mover txt-mover-0" cx="59" cy="40" r="3"/>' +
            '<circle class="txt-mover txt-mover-0 txt-mover-b" cx="59" cy="40" r="3"/>' +
            '<circle class="txt-mover txt-mover-1" cx="167" cy="40" r="3"/>' +
            '<circle class="txt-mover txt-mover-1 txt-mover-b" cx="167" cy="40" r="3"/>' +
    ...
    function mount(host, cfg) {
        ...
        '<section class="txt" data-kind="' + esc(cfg.kind || 'transfer') + '" data-state="running" aria-live="polite">' +
          ...
          (cfg.interactive
            ? '<div class="txt-media" id="' + uid + 'Media" hidden></div>'
            : '') +
          ...
          (cfg.devControls === false ? '' :
            '<div class="txt-demo">' +
              '<p class="txt-demo-label">Prototype: simulate a transaction state</p>' +
        ...
        var mediaShown = false, mediaIdx = 0, confSince = null;
        ...
        function paintMedia() {
            ...
            mediaEl.innerHTML =
                '<p class="txt-media-label">' + esc(cfg.interactive.label || 'A closer look while you wait') + '</p>' +
                '<div class="txt-media-card">' +
                  (f.art || '') +
                  '<p class="txt-media-title">' + f.title + '</p>' +
                  '<p class="txt-media-body">' + f.body + '</p>' +
                  '<div class="txt-media-nav">' +
                    '<button type="button" class="txt-media-btn" data-m="prev"' + (mediaIdx === 0 ? ' disabled' : '') + '>Back</button>' +
                    '<span class="txt-media-count">' + (mediaIdx + 1) + ' of ' + frames.length + '</span>' +
                    '<button type="button" class="txt-media-btn" data-m="next"' + (mediaIdx === frames.length - 1 ? ' disabled' : '') + '>Next</button>' +
                  '</div>' +
                '</div>';
        ...
        function maybeShowMedia() {
            ...
            if (!stages[idx] || !stages[idx].conf) { confSince = null; return; }
        ...
        function goDelayed() {
            ...
            bannerEl.innerHTML = ICON.clock + '<span>' + (cfg.delayedCopy ||
                'Networks get busy. Your money is safe and still queued. Nothing to resend or pay again.') + '</span>';
        ...
        function goFailed() {
            ...
            bannerEl.innerHTML = ICON.alert + '<span>' + (cfg.failCopy ||
                'The network rejected it. No funds left your wallet. Try again, or contact support.') + '</span>';
            bannerEl.hidden = false;
        ...
        function advance() {
            ...
            if (idx >= stages.length - 1) { later(goDone, stageMs[idx] || 600); return; }
