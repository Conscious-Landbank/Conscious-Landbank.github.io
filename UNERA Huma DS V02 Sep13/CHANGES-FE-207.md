...
> 23 Aug, second revision: the pending-transaction strip is removed from wallet-enhanced.html and dashboard-enhanced.html (Renol). The KYC banner returns to its original place above the page header. Pending transactions still surface through the nav pending pill and the bell notification; the full tracker stays on the donate, buy and swap status steps.
