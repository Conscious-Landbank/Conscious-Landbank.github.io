# Edge-case demo bar, the canonical pattern
...
2. In-memory only. Never write demo state to `localStorage`/`sessionStorage`. A refresh
   always returns to the happy path. (Real state such as KYC and wallet may persist; the *demo
   selector* never does.)
...
8. Never use `hidden` alone on a flex or grid banner: a class with `display:flex` overrides the
...
<div class="flow-demo-bar" id="xxxDemoBar">
    <p class="demo-label">Prototype: simulate a &lt;thing&gt;</p>
