# UNERA — Notification Services (captured PRD)
...
> source of truth for notification work on the UNERA **Platform** (consumer app).

**Purpose:** Defines user-facing notification requirements for two *separate* products —
the **UNERA Stablecoin** (issuer portal) and the **UNERA Platform** (consumer
exchange/wallet app). Notifications are **not reusable across the two**. Each product owns
its own events, templates, records, channels, and UI rules. The `unera-pages/` consumer app
is the **UNERA Platform** product — use §5.2 events and platform journey language.
...
## 5.2 UNERA Platform Required Events (THIS app)
...
| NOTI-UP-05 | Stablecoin service announcement surfaced on platform | info OR warning | Stablecoin notice affects UNERA users | Inform of impact w/o reusing Stablecoin records |
...
- **UNERA copy uses platform journey language, not issuer language** (no mint/burn/cash-out — AC-03).
...
- **Browser tab (7.5.2.4):** document title shows unread count, e.g. "(3) UNERA Dashboard"; revert when all read; optional favicon dot.
...
- AC-01 Stablecoin & UNERA notifications stored as separate records/IDs.
- AC-02 Mint completion supports fiat- AND crypto-funded issuance. *(issuer portal)*
- AC-03 **UNERA never shows mint/burn/cash-out terminology in user copy.**
