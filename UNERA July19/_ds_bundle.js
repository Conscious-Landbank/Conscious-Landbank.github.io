...
try { (() => {
/* UNERA — Donate flow controller (fiat + crypto + multi-centre). Prototype state is in-memory only. */
(function () {
  'use strict';
  ...
  var BLOCKERS = [{
    ...
    id: 'no_receipt',
    label: 'Receipt not eligible'
  }, {
    id: 'inactive',
    label: 'Centre becomes inactive'
  }, {
    id: 'server',
  ...
  var multi = false;
  var selected = []; // centre ids
  var centreQuery = ''; // centre search filter (step 1)
  ...
  function promptCount() {
    return method === 'fiat' ? 0 : token === 'ETH' ? 1 : 2;
  }
  ...
  function totalUSD() {
    return method === 'fiat' ? totalTokenAmount() : totalTokenAmount() * quoteRate;
  }
  ...
  function goToStep(n, opts) {
    ...
    var TITLES = ['Centre', 'Amount', 'Review', 'Complete'];
  ...
  function renderFlowContext(n, opts) {
    ...
    var name = multi && selected.length > 1 ? 'Split across ' + selected.length + ' centres' : D.esc(c.name) + ' · ' + D.esc(c.country);
    fc.innerHTML = img + '<span class="fc-body"><span class="fc-label">Donating to</span><span class="fc-name">' + name + '</span></span>' + '<button type="button" class="btn btn-secondary btn-sm" id="fcChangeBtn">Change centre</button>';
  ...
  function renderCentres() {
    ...
    var list = D.CENTRES.filter(function (c) {
    ...
    pick.innerHTML = list.map(function (c) {
      ...
      var dis = !c.active;
      return '<button type="button" class="centre-pick-row' + (sel ? ' selected' : '') + '" role="option" aria-selected="' + sel + '" data-id="' + c.id + '"' + (dis ? ' disabled' : '') + '>' + '<span class="cp-img" style="background-color:' + c.tint + ';" aria-hidden="true"><img src="' + c.image.replace('w=800', 'w=200') + '" alt="" loading="lazy"></span>' + '<span class="cp-body"><span class="cp-name">' + D.esc(c.name) + (dis ? ' <span class="hc-inactive-badge">Paused</span>' : '') + '</span><span class="cp-meta">' + D.esc(c.country) + ' · ' + D.fmtUSD(c.totalDonatedUSD) + ' donated · ' + D.fmtNum(c.livesImpacted) + ' lives impacted</span></span>' + '<svg class="cp-check" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="20 6 9 17 4 12"/></svg>' + '</button>';
    }).join('');
        ...
        if (multi) {
          ...
          if (i === -1) {
            if (selected.length >= MAX_CENTRES) {
              var mh = $('multiHint');
              if (mh) {
                mh.textContent = 'You can split one donation across up to ' + MAX_CENTRES + ' centres.';
    ...
    var total = D.CENTRES.length;
    var countEl = $('centreCount');
    if (countEl) countEl.textContent = q ? list.length + ' of ' + total + ' centres match “' + centreQuery.trim() + '”' : total + ' centres available';
  ...
  function renderTokens() {
    $('tokenSelect').innerHTML = D.BALANCES.tokens.map(function (t) {
      return '<button type="button" class="token-chip' + (t.symbol === token ? ' selected' : '') + '" role="radio" aria-checked="' + (t.symbol === token) + '" data-t="' + t.symbol + '">' + t.symbol + ' <span style="font-weight:400;color:var(--text-secondary);">' + t.amount.toLocaleString('en-US', {
        maximumFractionDigits: t.native ? 4 : 2
      }) + '</span></button>';
    }).join('');
  ...
  function renderAmountUI() {
    ...
    if (crypto && walletOk) {
      quoteRate = price();
      $('amountLabel').textContent = multi ? 'Allocation per centre (' + token + ')' : 'Donation amount (' + token + ')';
      $('amountPrefix').textContent = token;
      $('availLine').hidden = false;
      $('availAmt').textContent = tok().amount.toLocaleString('en-US', {
        maximumFractionDigits: tok().native ? 4 : 2
      }) + ' ' + token;
      ...
      $('conversionTipText').innerHTML = 'All donated crypto is <strong>converted to USD</strong> before it is routed to the centre. Current quote: 1 ' + token + ' = ' + D.fmtUSD(quoteRate, token === 'ETH' ? 2 : 4) + '.';
      ...
      $('conversionTipText').textContent = 'Your card is charged in USD and the full donation is routed to the centre. Credit and debit cards are supported.';
    ...
    $('quickAmounts').innerHTML = quicks.map(function (q) {
      return '<button type="button" class="quick-amt" data-q="' + q + '">' + (method === 'fiat' ? '$' + q : q + ' ' + token) + '</button>';
    }).join('');
  ...
  function validateAmount() {
    ...
    if (method === 'crypto') {
      ...
      })) return fail('Each centre allocation must be at least ' + D.fmtUSD(MIN_USD) + '.');
    }
    return true;
  }
  ...
  function receiptEligible() {
    return blocker !== 'no_receipt';
  }
  function updateAmountLive() {
    ...
    rows += '<div class="summary-row summary-row--total"><span class="summary-row-label">Centre receives</span><span class="summary-row-value">' + D.fmtUSD(usd, 2) + '</span></div>';
    var hc = D.getCentre(selected[0]);
    if (hc.impactHints && hc.impactHints.length) {
      var hint = null;
      hc.impactHints.forEach(function (h) {
  ...
  function renderReview() {
    ...
    if (multi) {
      ...
      var c = D.getCentre(selected[0]);
      orderRows += row('Humanity Centre', D.esc(c.name) + ' · ' + D.esc(c.country));
      orderRows += row('Centre wallet', D.shortAddr(c.wallet), {
        mono: true
      });
    }
    ...
    if (method === 'crypto') {
      ...
      orderRows += row('Centre receives (USD)', D.fmtUSD(usd, 2), {
        total: true
      });
    } else {
      orderRows += row('Centre receives (USD)', D.fmtUSD(usd, 2), {
    ...
    if (method === 'fiat') {
      ...
      costRows += row('UNERA fee', 'None');
      costRows += row('Total charged to card', D.fmtUSD(usd, 2), {
        total: true
      });
    } else if (token === 'ETH') {
      var g1 = D.GAS.donateETH;
      costRows += row('Network fee — donate', g1.toFixed(5) + ' ETH · ≈ ' + D.fmtUSD(gasUSD(g1), 2));
      costRows += row('Total network fees', g1.toFixed(5) + ' ETH · ≈ ' + D.fmtUSD(gasUSD(g1), 2), {
        total: true
      });
    } else {
      var ga = D.GAS.approveETH,
        gd = D.GAS.donateETH;
      ...
      costRows += row('Total network fees', (ga + gd).toFixed(5) + ' ETH · ≈ ' + D.fmtUSD(gasUSD(ga + gd), 2), {
        total: true
      });
    }
    ...
    var n = promptCount();
    $('promptCountText').innerHTML = n === 0 ? 'No wallet confirmations needed — this is a card payment.' : n === 1 ? 'This needs <strong>1 wallet confirmation</strong> — the donation itself.' : 'This needs <strong>2 wallet confirmations</strong> — approve, then ' + (multi ? 'donateMulti' : 'donate') + '.';
    ...
    if (blocker === 'inactive') block = {
      t: 'This centre is no longer accepting donations',
      x: 'The centre was paused while you were reviewing. Please choose another centre.'
    };
  ...
  /* ── Confirm → processing → terminal ── */
  var serverErrShown = false;
  function chips(list) {
    $('txSeq').innerHTML = list.map(function (c) {
      return '<span class="tx-seq-chip ' + c.state + '">' + (c.state === 'done' ? '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="20 6 9 17 4 12"/></svg>' : '') + c.label + '</span>';
    }).join('');
  }
  function confirmDonation() {
    ...
    var c = D.getCentre(selected[0]);
    var refLabel = multi ? selected.length + ' centres' : c.name;
    ...
    var seq = method === 'fiat' ? [{
      ...
      label: '② Routing to centre',
      state: ''
    }] : promptCount() === 2 ? [{
      label: '① Approve ' + token,
      state: 'current'
    }, {
      label: '② ' + (multi ? 'donateMulti' : 'Donate'),
      state: ''
    }, {
    ...
    $('processingTitle').textContent = method === 'fiat' ? 'Processing your payment…' : 'Waiting for your wallet…';
    $('processingSub').textContent = method === 'fiat' ? 'Order ' + donationId + ' — payment instructions expire in 15 minutes if the payment isn’t completed.' : 'Confirm each step in your wallet. Don’t close this page.';
    if (method === 'fiat') D.notifyDonation('info', 'Payment instructions ready', 'Order ' + donationId + ' was created and is waiting for your card payment. It expires in 15 minutes.', 'Ref ' + donationId);
    chips(seq);
    var t = 1400;
    if (outcome === 'rejected' && method === 'crypto') {
      setTimeout(function () {
        terminal('rejected', donationId);
      }, t);
      return;
    }
    if (outcome === 'pay_failed' && method === 'fiat') {
      setTimeout(function () {
        terminal('pay_failed', donationId);
      }, t);
      return;
    }
    setTimeout(function () {
      seq[0].state = 'done';
      if (seq[1]) seq[1].state = 'current';
      chips(seq);
      D.notifyDonation('progressing', 'Donation awaiting confirmation', (method === 'fiat' ? 'Payment received — ' : 'Transaction submitted — ') + 'final confirmation is in progress.', 'Ref ' + donationId);
      setTimeout(function () {
        if (outcome === 'reverted' && method === 'crypto') return terminal('reverted', donationId);
        if (outcome === 'expired' && method === 'fiat') return terminal('expired', donationId);
        if (outcome === 'pay_failed') return terminal('pay_failed', donationId);
        if (outcome === 'rejected') return terminal('reverted', donationId);
        if (outcome === 'awaiting') return terminal('awaiting', donationId);
        if (outcome === 'timeout') return terminal('timeout', donationId);
        if (method === 'crypto') {
          // Conversion-pending state (DON-CRYPTO-05) before completion
          seq[seq.length - 2].state = 'done';
          seq[seq.length - 1].state = 'current';
          chips(seq);
          $('processingTitle').textContent = 'Converting to USD…';
          $('processingSub').textContent = 'Your donation is confirmed on-chain. Converting to USD and routing to the centre.';
          setTimeout(function () {
  ...
  function terminal(kind, donationId) {
    ...
    var html = '';
    if (kind === 'success') {
      D.notifyDonation('completed', 'Donation completed', 'Your donation of ' + D.fmtUSD(usd, 2) + ' to ' + (multi ? selected.length + ' centres' : c.name) + ' is confirmed. Thank you.', 'Ref ' + donationId);
      setTimeout(function () {
        D.notifyDonation('info', 'Tax receipt available', 'Your receipt for donation ' + donationId + ' is ready to download.', 'Ref ' + donationId, 'donation-history.html', 'Download receipt');
      }, 2200);
      setTimeout(function () {
        D.notifyDonation('completed', 'UYT earned from donation', 'You earned ≈ ' + (usd * 0.05).toFixed(1) + ' UYT from this donation.', 'Ref ' + donationId, 'donations.html', 'View impact');
      }, 3800);
      html = '<div class="amount-section terminal-state flow-card-animated">' + '<div class="success-icon-animated"><svg class="unera-checkmark" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M5 13l4 4L19 7"></path></svg>' + '<div class="lightning-badge"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 -960 960 960" fill="currentColor" aria-hidden="true"><path d="m320-80 40-280H160l360-520h80l-40 320h240L400-80h-80Z"/></svg></div></div>' + '<h2 class="terminal-title success" tabindex="-1" id="terminalHeading">Donation Complete — Thank You!</h2>' + '<p class="terminal-sub">' + (method === 'crypto' ? 'Your crypto was converted to USD and routed to the centre.' : 'Your card payment was routed to the centre.') + '</p>' + '<div class="success-details">' + detailRow('Donation ID', donationId) + detailRow('Date', D.fmtDate(new Date().toISOString())) + detailRow('Status', D.statusChip('completed')) + detailRow(multi ? 'Humanity Centres' : 'Humanity Centre', centreLabel) + detailRow('Source', method === 'fiat' ? 'Card (fiat)' : 'Crypto — ' + token) + (method === 'fiat' && getCard(cardId) ? detailRow('Card', D.esc(cardLabel(getCard(cardId)))) : '') + (method === 'crypto' ? detailRow('You donated', amt.toLocaleString('en-US', {
        maximumFractionDigits: 4
      }) + ' ' + token) : '') + detailRow('Centre received (USD)', '<span style="color:var(--fin-up);font-weight:700;">' + D.fmtUSD(usd, 2) + '</span>') + (hash ? detailRow('Transaction', '<span class="success-detail-value mono">' + explorerLink(hash) + '</span>') : '') + detailRow('Tax receipt', 'Being generated — we’ll notify you when it’s ready') + detailRow('UYT earned', '≈ ' + (usd * 0.05).toFixed(1) + ' UYT · <span class="uyt-pill">pending confirmation</span>') + '</div>' + '<div class="btn-actions"><a href="donation-history.html" class="btn btn-secondary">View donation history</a><a href="explore-centres.html" class="btn btn-primary">Back to centres</a></div>' + '</div>';
      renderTerminal(html, {
        allDone: true
      });
      return;
    ...
    if (kind === 'awaiting' || kind === 'timeout') {
      var subCopy = kind === 'timeout' ? 'This is taking longer than expected. We’ll update the status when confirmation is available — your donation stays safely pending and you can leave this page.' : method === 'crypto' ? 'Your transaction was submitted. We’ll notify you as soon as it’s confirmed on-chain and converted to USD — you can leave this page safely.' : 'Your payment was received. We’ll notify you as soon as the donation is confirmed and routed to the centre — you can leave this page safely.';
      html = terminalShell('pending', kind === 'timeout' ? 'Still Waiting for Confirmation' : 'Donation Awaiting Confirmation', subCopy, detailRow('Donation ID', donationId) + detailRow('Status', D.statusChip('awaiting_confirmation')) + detailRow(multi ? 'Humanity Centres' : 'Humanity Centre', centreLabel) + detailRow('Amount', method === 'fiat' ? D.fmtUSD(usd, 2) : amt + ' ' + token + ' · ≈ ' + D.fmtUSD(usd, 2)) + (hash ? detailRow('Transaction', '<span class="success-detail-value mono">' + explorerLink(hash) + '</span>') : ''), '<button type="button" class="btn btn-secondary" id="refreshStatusBtn">Refresh status</button><a href="donation-history.html" class="btn btn-secondary">Track in history</a><a href="donations.html" class="btn btn-primary">Back to Donations</a>');
      renderTerminal(html, {});
    ...
    var fails = {
      pay_failed: {
        ...
        x: 'We could not process the payment right now. Your card was not charged. Please try again or use a different card.',
        notify: true
      },
      expired: {
        t: 'This donation request expired',
        x: 'This donation request expired before payment was completed. Please start a new donation.',
        notify: true
      },
      reverted: {
        t: 'Donation couldn’t be completed',
        x: 'Unable to complete this donation — the transaction was reverted and no funds reached the centre. Please try again.',
        notify: true
      },
      rejected: {
        t: 'Donation not submitted',
        x: 'Donation was not submitted because the wallet request was rejected. Nothing left your wallet.',
        notify: false
      }
    };
    var f = fails[kind];
    if (f.notify) D.notifyDonation('error', 'Donation failed', f.x, 'Ref ' + donationId, 'donate.html', 'Try again');
    html = terminalShell(kind === 'rejected' ? 'pending' : 'fail', f.t, f.x + (f.notify ? ' If this keeps happening, contact support.' : ''), detailRow('Donation ID', donationId) + detailRow('Status', D.statusChip(kind === 'expired' ? 'expired' : kind === 'rejected' ? 'pending_payment' : 'failed')) + detailRow(multi ? 'Humanity Centres' : 'Humanity Centre', centreLabel), '<a href="explore-centres.html" class="btn btn-secondary">Back to centres</a><button type="button" class="btn btn-primary" onclick="location.reload()">Try again</button>');
    renderTerminal(html, {
      fail: true
    });
  }
  ...
  renderTokens();
  renderAmountUI();
  var centresLink = document.getElementById('navLinkCentres');
...
try { (() => {
/* UNERA — Humanity Centres & Donation shared prototype data + user-state module.
   PRD: docs "UNERA - Human Centres & Donation". Prototype-only; demo state is in-memory (never persisted). */
(function () {
  'use strict';

  var CENTRES = [{
    id: 'hc-nairobi',
    name: 'Kibera Community Centre',
    country: 'Kenya',
    region: 'Africa',
    active: true,
    ...
    about: 'Kibera Community Centre provides quality education, clean water access and daily school meals to more than 500 children in Africa\u2019s largest informal settlement. The centre pairs classroom learning with family counselling and community-led water infrastructure, so progress made in school is backed up at home.',
    programsDetail: [{
    ...
    milestones: [{
      year: "2018",
      text: "Centre founded; the first 120 children enroll."
    }, {
      year: "2020",
    ...
    galleryCaptions2: ["Children reading in the library corner", "A family counselling session", "The centre kitchen preparing lunch"],
    ...
    image: 'https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=800&q=80',
    gallery: ['https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80', 'https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80', 'https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80'],
    about: 'Tondo Youth Centre helps young people in Manila\u2019s Tondo district move from precarious work into stable trades. Vocational workshops, one-to-one mentorship and typhoon-readiness training have reached more than 800 youth since 2016.',
    programsDetail: [{
    ...
    story: {
      name: "Mark, age 19",
      text: "Mark left school at 15 to scavenge alongside his family. He enrolled in the centre's carpentry track, was paired with a mentor, and earned a national skills certificate within a year. He now apprentices at a furniture workshop and mentors two younger trainees himself. Your gift keeps that pipeline open."
    },
    outcomes: [{
      value: "73%",
    ...
    milestones: [{
      year: "2016",
      text: "Centre opens with a single carpentry workshop and 40 trainees."
    }, {
      year: "2019",
    ...
    image: 'https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=800&q=80',
    gallery: ['https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80', 'https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80', 'https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80'],
    about: 'El Alto Family Centre brings primary health services, licensed childcare and nutrition programs to families at 4,000 m in Bolivia\u2019s fastest-growing city. Mobile clinics reach neighbourhoods that have no permanent health post.',
    programsDetail: [{
      name: 'Family Health',
    ...
    milestones: [{
      year: "2019",
      text: "Centre opens; the first mobile-clinic route launches."
    }, {
      year: "2021",
    ...
    image: 'https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=800&q=80',
    gallery: ['https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80', 'https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80', 'https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80'],
    about: 'Colombo Coastal Centre supports fishing families on Sri Lanka\u2019s west coast with livelihood grants, sustainable-fisheries training and rapid flood response. After the 2024 floods the centre helped rebuild 60 homes and restore 200 livelihoods.',
    programsDetail: [{
    ...
    milestones: [{
      year: "2017",
      text: "Centre founded to support coastal fishing cooperatives."
    }, {
      year: "2020",
    ...
    story: {
      name: "Efua, age 27",
      text: "Efua ran a market stall with no way to track her money. She took the centre's bookkeeping and digital-skills courses at night; within a year she had formalised her business, opened a second stall and hired her first employee. Your gift funds the next Efua's seat in the lab."
    },
    outcomes: [{
      value: "400+",
    ...
    about: 'Dhaka Health Outreach operates mobile clinics and maternal-health visits across greater Dhaka. The centre is temporarily not accepting new donations; existing program commitments continue to be honoured.',
    programsDetail: [{
      name: 'Mobile Clinics',
    ...
    impactHints: [],
    lead: "Across greater Dhaka, Dhaka Health Outreach brings mobile clinics and maternal-health visits to neighbourhoods without a permanent health post. The centre is temporarily not accepting new donations; existing program commitments continue to be honoured.",
    story: {
      name: "Shirin, new mother",
      text: "Shirin had no antenatal care until a centre midwife began home visits on her lane. Regular checkups carried her through a safe delivery, and her newborn started life with a full immunisation record."
    },
    outcomes: [{
      value: "9,640",
    ...
    milestones: [{
      year: "2015",
      text: "Centre founded; the first mobile-clinic route launches."
    }, {
      year: "2019",
    ...
    update: {
      date: "Notice",
      text: "This centre is not accepting new donations right now. Existing program commitments continue to be honoured."
    },
    ...
    id: 'hc-jakarta',
    name: "Jakarta Coastal Resilience Centre",
    country: "Indonesia",
    region: "Asia",
    active: true,
    ...
    image: "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=800&q=80",
    gallery: ["https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80", "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80", "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80"],
    about: "Jakarta Coastal Resilience Centre helps families in North Jakarta live with rising water. The centre builds community flood defences, restores clean-water access and helps fishing households diversify their income before the next tidal flood arrives.",
    programsDetail: [{
      "name": "Flood Defence",
    ...
    milestones: [{
      "year": "2019",
      "text": "Centre opens; first community flood barrier built."
    }, {
      "year": "2021",
    ...
    id: 'hc-kampala',
    name: "Kampala Girls' Education Centre",
    country: "Uganda",
    region: "Africa",
    active: true,
    ...
    image: "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=800&q=80",
    gallery: ["https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80", "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80", "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80"],
    about: "Kampala Girls' Education Centre keeps girls in the classroom through the years they are most likely to drop out. Scholarships, one-to-one mentors and school health support help students finish secondary school and plan what comes next.",
    programsDetail: [{
      "name": "Scholarships",
    ...
    milestones: [{
      "year": "2017",
      "text": "Centre opens with 60 scholarships."
    }, {
      "year": "2020",
    ...
    id: 'hc-amman',
    name: "Amman Refuge Support Centre",
    country: "Jordan",
    region: "Middle East",
    active: true,
    ...
    image: "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=800&q=80",
    gallery: ["https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80", "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80", "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80"],
    about: "Amman Refuge Support Centre helps refugee families rebuild a footing in a new city. Language and vocational classes, safe childcare and primary health support give parents the room to find work and children the stability to keep learning.",
    programsDetail: [{
    ...
    testimonial: {
      ...
      "who": "Omar, centre graduate and volunteer"
    },
    galleryCaptions: ["A language class in session", "The centre childcare room", "A primary-health consultation"],
    ...
    milestones: [{
      "year": "2018",
      "text": "Centre opens with language and childcare services."
    }, {
      "year": "2020",
    ...
    id: 'hc-lima',
    name: "Lima Highland Health Centre",
    country: "Peru",
    region: "South America",
    active: true,
    ...
    image: "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=800&q=80",
    gallery: ["https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80", "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80", "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80"],
    about: "Lima Highland Health Centre brings maternal care, child nutrition and mobile clinics to families on the steep, under-served edges of the city. Trained health workers reach homes that sit hours from the nearest permanent clinic.",
    programsDetail: [{
      "name": "Maternal Care",
    ...
    milestones: [{
      "year": "2020",
      "text": "Centre opens; first mobile-clinic route launches."
    }, {
      "year": "2022",
    ...
    id: 'hc-hanoi',
    name: "Hanoi Digital Futures Centre",
    country: "Vietnam",
    region: "Asia",
    active: true,
    ...
    image: "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=800&q=80",
    gallery: ["https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80", "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80", "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80"],
    about: "Hanoi Digital Futures Centre helps young people turn digital skills into steady income. Structured coding tracks, practical vocational courses and micro-business support open doors that a traditional CV alone often cannot.",
    programsDetail: [{
    ...
    galleryCaptions2: ["A mentor reviewing a portfolio", "A demo-day pitch session", "The centre computer lab"],
    ...
    milestones: [{
      "year": "2021",
      "text": "Centre opens with two computer labs."
    }, {
      "year": "2023",
    ...
    id: 'hc-capetown',
    name: "Cape Town Early Years Centre",
    ...
    image: "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=800&q=80",
    gallery: ["https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80", "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80", "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=400&q=80"],
    about: "Cape Town Early Years Centre gives children the strongest possible start before school begins. Play-based early education, daily nutrition and family support build the foundations that shape a child’s whole education.",
    programsDetail: [{
    ...
    galleryCaptions: ["A play-based learning session", "The centre mealtime", "A parenting workshop"],
    ...
    milestones: [{
      "year": "2016",
      "text": "Centre opens with one early-learning classroom."
    }, {
      "year": "2019",
  ...
  function getCentre(id) {
    return CENTRES.find(function (c) {
      return c.id === id;
    }) || CENTRES[0];
  }
  function qs(name) {
  ...
  /* Brand graphic-device artwork (organic blobs) — the DS illustration language; no figurative photos.
     Deterministic per centre so each card/hero looks distinct but on-brand. */
  var BLOB_SETS = [[{
    w: 78,
    h: 78,
  ...
  function centreIndex(id) {
    return CENTRES.findIndex(function (c) {
      return c.id === id;
    });
  }
  window.UNERA_DON = {
...
try { (() => {
/* ============================================================================
 * UNERA — Common pre-submit gas / balance validation for TOKEN transactions
 * ----------------------------------------------------------------------------
...
try { (() => {
/* UNERA — native <select> → wallet-scope dropdown enhancer.
