...
var BLOCKERS = [
  ...
  { id: 'inactive', label: 'Centre becomes inactive' },
...
function renderFlowContext(n, opts) {
  ...
  var name = multi && selected.length > 1
    ? 'Split across ' + selected.length + ' centres'
  ...
  fc.innerHTML = img + '<span class="fc-body"><span class="fc-label">Donating to</span><span class="fc-name">' + name + '</span></span>'
    + '<button type="button" class="btn btn-secondary btn-sm" id="fcChangeBtn">Change centre</button>';
...
function renderCentres() {
      ...
      if (multi) {
        ...
        if (i === -1) {
          if (selected.length >= MAX_CENTRES) {
            var mh = $('multiHint');
            if (mh) { mh.textContent = 'You can split one donation across up to ' + MAX_CENTRES + ' centres.'; mh.classList.add('error'); mh.setAttribute('role', 'alert'); }
            return;
          }
          selected.push(id);
          var mh2 = $('multiHint');
  ...
  if (countEl) countEl.textContent = q
    ? (list.length + ' of ' + total + ' centres match “' + centreQuery.trim() + '”')
    : (total + ' centres available');
...
function validateAmount() {
  ...
  if (method === 'crypto') {
    ...
    if (multi && selected.some(function (id) { return (parseFloat(alloc[id]) || 0) * quoteRate < MIN_USD; })) return fail('Each centre allocation must be at least ' + D.fmtUSD(MIN_USD) + '.');
  }
  return true;
}
...
function renderReview() {
  ...
  if (multi) {
    ...
    orderRows += row('Centre wallet', D.shortAddr(c.wallet), { mono: true });
  }
  ...
  if (method === 'crypto') {
    ...
    orderRows += row('Centre receives (USD)', D.fmtUSD(usd, 2), { total: true });
  } else {
    orderRows += row('Centre receives (USD)', D.fmtUSD(usd, 2), { total: true });
  }
  ...
  var block = null;
  if (blocker === 'inactive') block = { t: 'This centre is no longer accepting donations', x: 'The centre was paused while you were reviewing. Please choose another centre.' };
...
function chips(list) {
  $('txSeq').innerHTML = list.map(function (c) {
    return '<span class="tx-seq-chip ' + c.state + '">' + (c.state === 'done'
      ? '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="20 6 9 17 4 12"/></svg>' : '') + c.label + '</span>';
  }).join('');
}
function confirmDonation() {
  ...
  var seq = method === 'fiat'
    ? [{ label: '① Card payment', state: 'current' }, { label: '② Routing to centre', state: '' }]
    : (promptCount() === 2
      ? [{ label: '① Approve ' + token, state: 'current' }, { label: '② ' + (multi ? 'donateMulti' : 'Donate'), state: '' }, { label: '③ Convert to USD', state: '' }]
      : [{ label: '① Donate (ETH)', state: 'current' }, { label: '② Convert to USD', state: '' }]);
  $('processingTitle').textContent = method === 'fiat' ? 'Processing your payment…' : 'Waiting for your wallet…';
  $('processingSub').textContent = method === 'fiat'
    ? 'Order ' + donationId + ' — payment instructions expire in 15 minutes if the payment isn’t completed.'
    : 'Confirm each step in your wallet. Don’t close this page.';
  ...
  if (outcome === 'rejected' && method === 'crypto') {
    setTimeout(function () { terminal('rejected', donationId); }, t);
    return;
  }
  if (outcome === 'pay_failed' && method === 'fiat') {
    setTimeout(function () { terminal('pay_failed', donationId); }, t);
    return;
  }
  setTimeout(function () {
    seq[0].state = 'done';
    ...
    setTimeout(function () {
      ...
      if (method === 'crypto') {
        ...
        setTimeout(function () {
          seq.forEach(function (x) { x.state = 'done'; });
          chips(seq);
          setTimeout(function () { terminal('success', donationId); }, 500);
        }, t);
        return;
      }
      seq.forEach(function (x) { x.state = 'done'; });
...
function terminal(kind, donationId) {
  ...
  if (kind === 'success') {
    ...
    setTimeout(function () { D.notifyDonation('completed', 'UYT earned from donation', 'You earned ≈ ' + (usd * 0.05).toFixed(1) + ' UYT from this donation.', 'Ref ' + donationId, 'donations.html', 'View impact'); }, 3800);
    html = '<div class="amount-section terminal-state flow-card-animated">'
      ...
      + '<p class="terminal-sub">' + (method === 'crypto' ? 'Your crypto was converted to USD and routed to the centre.' : 'Your card payment was routed to the centre.') + '</p>'
      ...
      + detailRow(multi ? 'Humanity Centres' : 'Humanity Centre', centreLabel)
      ...
      + detailRow('Centre received (USD)', '<span style="color:var(--fin-up);font-weight:700;">' + D.fmtUSD(usd, 2) + '</span>')
      ...
      + detailRow('UYT earned', '≈ ' + (usd * 0.05).toFixed(1) + ' UYT · <span class="uyt-pill">pending confirmation</span>')
      + '</div>'
      + '<div class="btn-actions"><a href="donation-history.html" class="btn btn-secondary">View donation history</a><a href="explore-centres.html" class="btn btn-primary">Back to centres</a></div>'
      + '</div>';
  ...
  if (kind === 'awaiting' || kind === 'timeout') {
    var subCopy = kind === 'timeout'
      ...
      : (method === 'crypto'
        ...
        : 'Your payment was received. We’ll notify you as soon as the donation is confirmed and routed to the centre — you can leave this page safely.');
  ...
  var fails = {
    ...
    reverted: { t: 'Donation couldn’t be completed', x: 'Unable to complete this donation — the transaction was reverted and no funds reached the centre. Please try again.', notify: true },
  ...
  html = terminalShell(kind === 'rejected' ? 'pending' : 'fail', f.t, f.x + (f.notify ? ' If this keeps happening, contact support.' : ''),
    ...
    + detailRow(multi ? 'Humanity Centres' : 'Humanity Centre', centreLabel),
    '<a href="explore-centres.html" class="btn btn-secondary">Back to centres</a><button type="button" class="btn btn-primary" onclick="location.reload()">Try again</button>');
