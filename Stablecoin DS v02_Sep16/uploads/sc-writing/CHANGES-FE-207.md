...
| `ui_kits/auth/index.html` | Product-name copy and `<title>`/tab titles now read **Unera Stablecoin**. | [Rename] |
| `ui_kits/auth/README.md` | Product name. | [Rename] |
| `ui_kits/stablecoin-app/README.md` | Product name, "yield to Humanity Centers". | [Rename] |
| `readme.md` | Product name, consumer layer becomes **Huma Platform**, Humanity Centres becomes **Centers**. | [Rename] |
...
| `screenshots/fe-207/*` | 28 new verification screenshots (14 screens × desktop/mobile). | n/a |
...
| Humanity Centres / Centre / Centres | **Humanity Centers / Humanity Center / Centers** | US spelling, incl. the PoR "yield to Humanity Centers" block. |
...
### 3.3 Copy for chips and timings
...
| 3 | Compliance checks | "A quick automatic check of your limits and quota, plus sanctions screening." |
...
| 2 | Checks | "A quick automatic check of your limits and quota, plus sanctions screening." |
...
- Your money is safe. The network is slow, not your funds.
...
- Portfolio attention card: the same information as a full-width card immediately under the balance header, with a "Track it" button.
...
| Rule | Implementation |
|---|---|
| **USDC / USDT only** | Segmented "Pay with" row on the amount step: USDC and USDT are selectable; ETH and BTC render as disabled dashed chips reading "ETH · Soon" / "BTC · Soon". `RATES` now holds only `{ USDC: 1, USDT: 1 }`. |
...
2. What needs attention: the pending transaction card, then the account/service state banner. On the portfolio screen the global banner is suppressed and re-rendered here, so it is never shown twice.
3. Holdings by network + recent activity
4. Learn and proof: "What backs your hUSD" with a quiet link to Proof of reserve
