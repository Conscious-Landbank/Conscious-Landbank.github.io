# FE-208: Humanity Centers and donation features, Unera Stablecoin side
...
| `_audit/FE-208-audit.md` | New. Inventory of every HC / donation / yield / Huma Platform / points mention, the Met/Partial/Missing scorecard against brief §D with evidence, the implementation plan, and four open questions. | n/a | n/a |
| `screenshots/fe-208/*` | New. 10 verification screenshots (5 views × desktop/mobile). | n/a | n/a |
...
## 2. Passive Reserve Yield block, before and after [Stream]
...
`An independent issuer, by design` kept its three prose columns, which are the *issuer's* independence claims
and still earn their place, with one correction:

- **"Reserve yield, kept in-layer"** becomes **"Reserve yield, routed by rule"**. The yield does not stay in the
...
Below it, the three bare pills became three labelled cells carrying a name, the participants and each
layer's actual role (D-11):

| Layer | Participants | Role as stated |
|---|---|---|
| **Stablecoin layer** *(you are here)* | Unera Stablecoin · hUSD | Issues and redeems hUSD, holds the reserve and proves it, runs its own KYC. |
...
- **Notification** (D-1, D-4): one new seed item, `n7`, `info` / `announcement`, dated Aug 1, deep-linking to
  PoR: *"Reserve yield routed to Humanity Centers"* / *"$1.42M of Passive Reserve Yield for July reached
  Humanity Centers automatically. The Swiss Association allocates it; every transfer is on-chain."* CTA "See the routing".
- **"Notification centre" becomes "Notification center"** (D-8b): the last British spelling in visible copy, three
...
reproduced on the `da86ab2` baseline before any FE-208 edit. It would have hidden most of the new copy, so
...
the portfolio grid, the hero, the coverage cards and the charts were re-checked at both viewports
for regressions.
...
**Zero console errors, zero 404s / failed local requests** on `UNERA hUSD Portal.dc.html`
(Proof of reserve, Notifications), `ui_kits/stablecoin-app/index.html` (PoR) and `ui_kits/auth/index.html`.

> **Test-rig note.** The canvas loads React 18.3.1, ReactDOM 18.3.1 and `@babel/standalone` 7.26.4 from
