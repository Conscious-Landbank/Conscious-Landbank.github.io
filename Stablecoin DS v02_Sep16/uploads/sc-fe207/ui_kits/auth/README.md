...
- **Remember Me is mobile-only** (7 days, absolute expiry, skips 2FA). Desktop ignores the flag
  — the checkbox only renders at mobile widths — and auto-logs-out after **30 min** inactivity.
...
- **2FA setup** (§4.4 + SAD-3) — email setup code (5-min expiry) → **Save your backup codes**
  (10 codes, download/copy, confirm-saved gate) → "Email 2FA is on".
...
- **Set new password** (§4.3) — new + confirm with live checklist and the "signs you out on
  all devices" outcome → **Password updated** (sign in again; no auto-login).
