...
Reserve yield (interest on HQLA holdings) funds portal operations and Humanity Centers.
...
- `stablecoin-app/` — the authenticated Portal (hUSD): Dashboard with KYC-tier card +
  ...
  **notification center**, **Issue** flow (fiat + USDC/USDT crypto onramp, **bidirectional**
  ...
  yield→Humanity Centers, contract address + network/decimals/status, attestations with
