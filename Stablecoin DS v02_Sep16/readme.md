...
performs exactly two transaction types: **Mint** (issuance) and **Burn** (redemption). Everything else
(swaps, remittance, payments, exchange, DeFi, governance) is a Platform-Layer concern and
never appears here.
...
- **Proof-first copy.** Every high-stakes action names its guarantees and shows its
  receipts: "Backed 1:1 by audited reserves", "On-chain transfers are irreversible. Double-check
  the address", "Recorded to block #21,456,789", Tx IDs, attestation dates,
...
- **Emoji.** Avoid them. The one sanctioned exception is the MetaMask fox glyph in the
  wallet-connect grid. No decorative emoji.
- **Vibe:** regulated issuer infrastructure: transparent, exact, quietly confident.
- **Writing.** Every human-facing string and every doc in this repo follows
  `skills/human-writing/SKILL.md`. Run
...
are solid Deep Blue, with no gradient text on product screens. KPI figures use
...
(`rgba(23,61,71,0.13)`), soft resting shadow (`--shadow-card`). On hover the border turns Deep
Blue, the shadow deepens (`--shadow-hover`), card **lifts** (`translateY(-3px)`). Buttons 6px, inputs/small-cards 8px, modals 10px; pills + avatars stay fully round.
...
**Transparency & blur.** Use it sparingly: the connect-modal overlay uses a light
...
- **Logos:** in `assets/logos/`, the clean lockups are the **nav variants**
...
CONNECT, then pill, then dropdown. Never skip the disconnected state.

## What's in this system

**Foundations (root):**
- `styles.css` is the single entry point consumers link (`@import` manifest).
- `tokens/colors.css` · `tokens/typography.css` · `tokens/spacing.css` ·
  `tokens/surfaces.css` carry all CSS custom properties + `@font-face`.
- `base.css` has the resets + foundational classes mirroring the React primitives.
- `fonts/` holds the TestFoundersGrotesk OTF family; `assets/logos/` holds the UNERA wordmarks.
...
- `core/` holds **Button**, **Badge**, **StatusPill**, **Card**, **Stepper**
- `forms/` holds **Input**, **Select**, **Checkbox**
- `stablecoin/` holds **WalletPill** (connect states), **ReserveBar** (PoR composition),
...
- `charts/` holds **ReserveGauge** (ratio vs 100% floor), **CompositionDonut**,
  **MaturityLadder** and **TrendChart** (90-day), all pure SVG with no libraries

**UI kits** (`ui_kits/`):
- `stablecoin-app/` is the authenticated Portal (hUSD): Dashboard with KYC-tier card +
  ...
  **notification center**, **Issue** flow (fiat + USDC/USDT crypto onramp, **bidirectional**
  ...
  (bank-only fiat; burn, then liquidity, then disbursement; honest ~1-day SLA), public **Proof of
  Reserve** (ratio gauge, composition donut, custodian + maturity, 90-day trend,
  **Passive Reserve Yield to Humanity Centers** (cumulative routed, Swiss Association as
...
- `auth/` covers login, signup (email-first), magic link, **email-based 2FA (+authenticator
  option)**, password reset, partner-framed verification, access-level success state.

**Motion** (`assets/motion/husd-motion.js`, 0908): the Movmint-inspired-but-softer motion
layer — `<husd-globe>` (hero dot-globe with gentle arcs), `<husd-flow>` (value-flow diagram
with travelling comet dots), `<husd-trend>` (draw-in chart with a travelling glow),
`<husd-reveal>` (rise-on-scroll). Powers the public **Overview/Mission** and **Use cases**
screens of the Portal (Kevin 09-08: ethena-style mission topology, PYUSD/USDC-style use
cases, movmint-style data & interaction, "more humane and softer"). All loops pause
offscreen; `prefers-reduced-motion` renders static frames.

**`_audit/`** holds **`Stablecoin-Audit-and-Changes-2026-06.md`**, the current feature audit,
...
- **PoR figures, custodians, attestations** are realistic placeholders. Wire them to the real
