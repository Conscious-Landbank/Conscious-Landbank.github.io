...
/**
 * StatusTimeline is the vertical state-machine tracker for an issuance or redemption.
 * It is the stage list inside the FE-207 **Transaction Tracker**; the tracker adds the
 * plain-language headline, ETA + elapsed timer, reassurance block and "while you wait"
 * content around it.
 *
 * FE-207 stage vocabulary (plain language, one human sentence per stage, and every
 * sentence says nothing is required from the user):
 *   crypto on-ramp: Deposit seen · Network confirmations · Compliance checks ·
 *                    hUSD minted to your confirmed wallet
 *   fiat on-ramp:   Payment received · Checks · hUSD minted
 *   redemption:     hUSD removed · Reserve liquidity · Bank payout
 * Never a raw hash or "12 confirmations" in a stage title. Confirmation depth belongs
 * in the tracker's secondary bar ("Network is double-checking · 4 of 12").
 *
 * Map item.state onto done | current | pending | queued | blocked. Never expose
 * compliance reason codes; "Unable to process" is the safe blocked label.
 * Show honest progress at every stage, including the slow ones.
 */
