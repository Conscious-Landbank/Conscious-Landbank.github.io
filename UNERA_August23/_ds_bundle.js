...
(function () {
  ...
  function feeUSD() {
    return D.processingFee(method, totalUSD());
  }
  function convFeeUSD() {
  ...
  function renderAmountUI() {
    ...
    if (crypto && walletOk) {
      ...
      $('amountHint').textContent = 'Donations between ' + D.fmtUSD(MIN_USD) + ' and ' + D.fmtUSD(MAX_USD) + ' (USD value) per transaction.';
      $('conversionTipText').innerHTML = f.code === 'USD' ? 'Your card is charged in USD and the donation is routed to the ' + D.SETTLEMENT.fiatAccountLabel + '. Fiat is never converted to crypto.' : 'Your card is charged in ' + f.code + '. The payment rail converts it to USD before routing. We keep your original amount, the rate and the USD value on the record.';
  ...
  function validateAmount() {
    ...
    if (method === 'fiat' && !cardId) return fail('Add a payment card to donate by fiat.');
    if (method === 'crypto') {
  ...
  function updateAmountLive() {
    ...
    if (ql) {
      if (method === 'crypto' && quote) {
        ql.hidden = false;
        ql.innerHTML = '<strong>≈ ' + D.fmtUSD(quote.usd, 2) + '</strong> USD value · quoted via <strong>Uniswap</strong>, best of the USDC and USDT routes' + (quote.converted ? ' · settles as <strong>' + quote.settlementAsset + '</strong>' : ' · settles as <strong>' + quote.settlementAsset + '</strong> (no swap)');
      } else {
        ql.hidden = true;
      }
    }
  ...
  function renderReview() {
    ...
    orderRows += row('Method', method === 'fiat' ? 'Donate by Fiat · card rail' : 'Donate by Crypto · ' + token);
    var amt = totalTokenAmount(),
      usd = totalUSD();
    if (method === 'fiat') {
      var sc = getCard(cardId);
      if (sc) orderRows += row('Card', D.esc(cardLabel(sc)));
      /* DON-FIAT-04 - non-USD keeps original currency, amount and rate on the record. */
      if (ccy !== 'USD') {
      ...
      /* DON-CRYPTO-03 - asset, original amount, USD quote, destination type, wallet address. */
      orderRows += row('You donate', amt.toLocaleString('en-US', {
        maximumFractionDigits: 6
      }) + ' ' + token);
      orderRows += row('USD quote', D.fmtUSD(usd, 2) + '<span class="summary-row-note">via Uniswap, best of the USDC and USDT routes</span>', {
        total: true
      });
      ...
      orderRows += row('Destination type', D.SETTLEMENT.multisigLabel);
      orderRows += row('Wallet address', D.shortAddr(D.SETTLEMENT.multisig), {
        mono: true
      });
      orderRows += row('Network', D.SETTLEMENT.network + '<span class="summary-row-note">More networks are being added.</span>');
    }
    orderRows += row('Tax receipt', receiptEligible() ? "Issued on confirmation, tied to this donation's transaction ID" : 'May not be available in your region');
    ...
    if (method === 'fiat') {
      ...
      if (ccy !== 'USD') costRows += row('Charged to your card', D.fmtCcy(ccy, totalChargedUSD() / ccyInfo().rate) + ' ' + ccy);
    } else {
      ...
      costRows += row('Total charged<span class="summary-row-note">Donation + processing fee. Network fees are paid separately from your wallet.</span>', D.fmtUSD(totalChargedUSD(), 2), {
        total: true
      });
    }
  ...
  function trackerConfig(donationId, c, refLabel) {
    ...
    var stages = crypto ? [{
      ...
      sub: 'The blockchain is writing your donation into a block.',
      conf: true
    }].concat(converts ? [{
      title: 'Converting to ' + settleAs,
      sub: 'Your ' + token + ' becomes ' + settleAs + ' at the quoted route. Automatic.'
    }] : []).concat([{
      title: 'Delivered to the ' + D.SETTLEMENT.multisigLabel,
      sub: 'Funds arrive. Your receipt is issued.'
    }]) : [{
      ...
      title: 'Routed to the settlement account',
      sub: "Sent on to this Center's settlement account."
    }, {
      title: 'Delivered',
      sub: 'Funds arrive. Your receipt is issued.'
    }];
    ...
    return {
      ...
      timings: {
        stages: converts ? [2600, 3200, 4200, 3600, 800] : [3200, 4200, 3600, 800]
      },
  ...
  function terminal(kind, donationId) {
    ...
    if (kind === 'conversion') {
      ...
      html = terminalShell('pending', 'Conversion in progress', D.ERR.converting, detailRow('Donation ID', donationId) + detailRow('Status', D.statusChip('conversion_pending')) + detailRow(multi ? 'Humanity Centers' : 'Humanity Center', centreLabel) + detailRow('You donated', amt.toLocaleString('en-US', {
        maximumFractionDigits: 6
      }) + ' ' + token) + detailRow('USD quote', D.fmtUSD(usd, 2) + ' <span class="summary-row-note">via Uniswap, best of the USDC and USDT routes</span>') + detailRow('Converting to', settleAsset + ' <span class="summary-row-note">Crypto Swap Worker</span>') + detailRow('Destination type', D.SETTLEMENT.multisigLabel) + (hash ? detailRow('Transaction', '<span class="success-detail-value mono">' + explorerLink(hash) + '</span>') : '') + detailRow('Tax receipt', 'Pending · issued once the conversion settles') + detailRow('Huma Points', hp.amount == null ? D.hpChip('unavailable') : '≈ ' + hp.amount + ' Huma Points · ' + D.hpChip('estimated')), '<a href="donation-history.html" class="btn btn-secondary">Track in donation history</a><a href="donations.html" class="btn btn-primary">Back to Donations</a>');
      renderTerminal(html, {});
      return;
    }
    if (kind === 'success') {
      ...
      setTimeout(function () {
        D.notifyDonation('info', 'Tax receipt available', 'Your receipt for donation ' + donationId + " is ready to download. It is tied to this donation's transaction ID.", 'Ref ' + donationId, 'donation-history.html', 'Download receipt');
      }, 2200);
      setTimeout(function () {
      ...
      html = '<div class="amount-section terminal-state flow-card-animated">' + '<div class="success-icon-animated"><svg class="unera-checkmark" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M5 13l4 4L19 7"></path></svg>' + '<div class="lightning-badge"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 -960 960 960" fill="currentColor" aria-hidden="true"><path d="m320-80 40-280H160l360-520h80l-40 320h240L400-80h-80Z"/></svg></div></div>' + '<h2 class="terminal-title success" tabindex="-1" id="terminalHeading">Donation complete. Thank you.</h2>' + '<p class="terminal-sub">' + (method === 'crypto' ? quote && quote.converted ? 'Your ' + token + ' was converted to ' + settleAsset + ' and settled to the ' + D.SETTLEMENT.multisigLabel + '.' : 'Your ' + token + ' settled directly to the ' + D.SETTLEMENT.multisigLabel + '.' : 'Your card payment settled to the ' + D.SETTLEMENT.fiatAccountLabel + '.') + '</p>' + '<div class="success-details">' + detailRow('Donation ID', donationId) + detailRow('Date', D.fmtDate(new Date().toISOString())) + detailRow('Status', D.statusChip('completed')) + detailRow(multi ? 'Humanity Centers' : 'Humanity Center', centreLabel) + detailRow('Source', method === 'fiat' ? 'Fiat · card rail' : 'Crypto · ' + token) + (method === 'fiat' && getCard(cardId) ? detailRow('Card', D.esc(cardLabel(getCard(cardId)))) : '') + (method === 'fiat' && ccy !== 'USD' ? detailRow('Original amount', D.fmtCcy(ccy, amt) + ' ' + ccy + ' · rate 1 ' + ccy + ' = ' + D.fmtUSD(ccyInfo().rate, 4)) : '') + (method === 'crypto' ? detailRow('You donated', amt.toLocaleString('en-US', {
        maximumFractionDigits: 6
      }) + ' ' + token) : '') + detailRow('Donation amount (USD)', '<span style="color:var(--fin-up);font-weight:700;">' + D.fmtUSD(usd, 2) + '</span>') + detailRow('Processing fee', '+ ' + D.fmtUSD(feeUSD(), 2)) + (convFeeUSD() ? detailRow('Conversion fee', D.fmtUSD(convFeeUSD(), 2)) : '') + detailRow('Total charged', D.fmtUSD(totalChargedUSD(), 2)) + detailRow('Settlement asset', settleAsset) + detailRow('Destination type', method === 'fiat' ? D.SETTLEMENT.fiatAccountLabel : D.SETTLEMENT.multisigLabel) + (hash ? detailRow('Transaction', '<span class="success-detail-value mono">' + explorerLink(hash) + '</span>') : '') + detailRow('Tax receipt', receiptEligible() ? 'Pending · being generated and tied to this donation\'s transaction ID' : D.ERR.noReceipt) + detailRow('Huma Points', hp.amount == null ? D.hpChip('unavailable') : '≈ ' + hp.amount + ' Huma Points · ' + D.hpChip('estimated')) + '</div>' + '<div class="btn-actions"><a href="donation-history.html" class="btn btn-secondary">View donation history</a><a href="explore-centres.html" class="btn btn-primary">Back to centers</a></div>' + '</div>';
      renderTerminal(html, {
        allDone: true
      });
      return;
...
(function () {
  ...
  function mount(host, cfg) {
    ...
    }).join('') + '</ol>' + (cfg.confirmations ? '<div class="txt-conf" id="' + uid + 'Conf" hidden>' + '<div class="txt-conf-head"><span class="txt-conf-label">' + esc(cfg.confirmations.label || 'Network is double-checking') + '</span>' + '<span class="txt-conf-count" id="' + uid + 'ConfCount">0 of ' + cfg.confirmations.target + '</span></div>' + '<div class="txt-conf-bar"><span class="txt-conf-fill" id="' + uid + 'ConfFill"></span></div>' + '</div>' : '') + '<div class="txt-reassure">' + (cfg.reassure || []).map(function (r, i) {
