...
**Responsiveness skill (mandatory):** every page task also follows `skills/responsive-mobile.md`. Eric reviews on a phone; zero horizontal overflow at 360 and 414 px on every step/modal/terminal, right-aligned wrapping values, one-line labels, thin full-width stacked CTAs, 44px+ targets, no hover-only controls. Apply while building and verify by DOM before hand-off, without being asked.
...
- **The compact balance hero is for the Unera Stablecoin portal only.** The consumer Wallet and Dashboard never lead with a balance figure, and carry no pending-transaction strip (pending state lives in the nav pill and the bell only): balances stay inside the Balances section, behind the show/hide toggle, hidden by default (`balancesHidden = true`, the button reads "Show Balances"). Decision: Renol, 23 Aug 2026.
...
- [ ] Balance hero ≤ 160px desktop, 2.5rem figure, exactly one primary CTA, attention strip directly beneath. Stablecoin portal only: on consumer Wallet and Dashboard there is no hero, and balances start hidden.
- [ ] Verified live via `eval_js` (computed styles, transitions, edge states), not assumed.
- [ ] Mobile pass per `skills/responsive-mobile.md`: no horizontal overflow at 360/414 px on any step, modal or terminal; row/value, CTA and chip standards met.
