...
issuance, redemption, and reserve-transparency surface of the UNERA ecosystem.
...
the Stablecoin Portal reads **institutional, audited, and proof-driven** — a custody
...
Reserve yield (interest on HQLA holdings) funds portal operations and Humanity Centres.
The Portal does **not** run governance, remittance, payments, exchange, or DeFi — those
live in the separate **UNERA Platform** layer and must never appear in stablecoin
licensing or UI.
...
- **Reserve language.** "Fully backed", "Reserve ratio", "1:1 redeemable",
  "Bank deposits / Stablecoins / Treasury bonds / Cash equivalents", "Independently
  audited", "Operational".
...
- **Signal Yellow `#ecd6a0`** is the Stablecoin signature accent and the system's
  ...
  (nav hover/active, badges on Deep Blue, accent CTAs, "fully backed" emphasis).
...
- `stablecoin/` — **WalletPill** (connect states), **ReserveBar** (PoR composition),
  **StatCard** (KPI figure), **StatusTimeline** (issuance/redemption state machine)
...
- `stablecoin-app/` — the authenticated Portal (hUSD): Dashboard with KYC-tier card +
  L1/L2 gating, **Issue** flow (fiat + USDC/USDT crypto onramp, live-rate preview, status
  timeline), **Redeem** (burn → liquidity → disbursement), public **Proof of Reserve**
  (ratio gauge, composition donut, custodian + maturity, 90-day trend, yield→Humanity
  Centres, contract address, quarterly attestations), **History** (mint/burn, filters).
- `auth/` — login, signup (email-first), magic link, **email-based 2FA (+authenticator
  option)**, password reset, Sumsub-framed verification, access-level success state.

**`_audit/PRD-gap-analysis.md`** — the PRD/spec gap analysis driving the V2 screens.
...
- **PoR figures, custodians, attestations** are realistic placeholders — wire to the real
  reserve pipeline / Sumsub / payment-rail provider for production.
