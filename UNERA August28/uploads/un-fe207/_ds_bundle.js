...
(function () {
  ...
  function setUserMenuActive(activePage) {
    ...
    if (activePage === 'centers') {
      var centers = document.getElementById('menuLinkCentres');
      if (centers) {
        centers.classList.add('active');
        centers.setAttribute('aria-current', 'page');
      }
  ...
  function isUserMenuItemActive(item, activePage) {
    if (activePage === 'centers' && item.label === 'Centers') return true;
...
(function () {
  ...
  function renderFlowContext(n, opts) {
    ...
    var name = multi && selected.length > 1 ? 'Split across ' + selected.length + ' centers' : D.esc(c.name) + ' · ' + D.esc(c.country);
  ...
  function renderReview() {
    ...
    if (multi) {
      ...
      orderRows += row('Center wallet', D.shortAddr(c.wallet), {
        mono: true
      });
    }
    ...
    if (method === 'crypto') {
      ...
      orderRows += row('Center receives (USD)', D.fmtUSD(usd, 2), {
        total: true
      });
    } else {
      orderRows += row('Center receives (USD)', D.fmtUSD(usd, 2), {
    ...
    if (blocker === 'inactive') block = {
      t: 'This center is no longer accepting donations',
      x: 'The center was paused while you were reviewing. Please choose another center.'
    };
    ...
    setTimeout(function () {
      ...
      setTimeout(function () {
        ...
        if (method === 'crypto') {
          ...
          $('processingSub').textContent = 'Your donation is confirmed on-chain. Converting to USD and routing to the center.';
          setTimeout(function () {
  ...
  function terminal(kind, donationId) {
    ...
    if (kind === 'success') {
      ...
      setTimeout(function () {
        D.notifyDonation('completed', 'Huma Points earned', 'You earned ≈ ' + (usd * 0.05).toFixed(1) + ' Huma Points from this donation.', 'Ref ' + donationId, 'donations.html', 'View impact');
      }, 3800);
      html = '<div class="amount-section terminal-state flow-card-animated">' + '<div class="success-icon-animated"><svg class="unera-checkmark" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M5 13l4 4L19 7"></path></svg>' + '<div class="lightning-badge"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 -960 960 960" fill="currentColor" aria-hidden="true"><path d="m320-80 40-280H160l360-520h80l-40 320h240L400-80h-80Z"/></svg></div></div>' + '<h2 class="terminal-title success" tabindex="-1" id="terminalHeading">Donation Complete — Thank You!</h2>' + '<p class="terminal-sub">' + (method === 'crypto' ? 'Your crypto was converted to USD and routed to the center.' : 'Your card payment was routed to the center.') + '</p>' + '<div class="success-details">' + detailRow('Donation ID', donationId) + detailRow('Date', D.fmtDate(new Date().toISOString())) + detailRow('Status', D.statusChip('completed')) + detailRow(multi ? 'Humanity Centers' : 'Humanity Center', centreLabel) + detailRow('Source', method === 'fiat' ? 'Card (fiat)' : 'Crypto — ' + token) + (method === 'fiat' && getCard(cardId) ? detailRow('Card', D.esc(cardLabel(getCard(cardId)))) : '') + (method === 'crypto' ? detailRow('You donated', amt.toLocaleString('en-US', {
        maximumFractionDigits: 4
      }) + ' ' + token) : '') + detailRow('Center received (USD)', '<span style="color:var(--fin-up);font-weight:700;">' + D.fmtUSD(usd, 2) + '</span>') + (hash ? detailRow('Transaction', '<span class="success-detail-value mono">' + explorerLink(hash) + '</span>') : '') + detailRow('Tax receipt', 'Being generated — we’ll notify you when it’s ready') + detailRow('Huma Points earned', '≈ ' + (usd * 0.05).toFixed(1) + ' Huma Points · <span class="uyt-pill">pending confirmation</span>') + '</div>' + '<div class="btn-actions"><a href="donation-history.html" class="btn btn-secondary">View donation history</a><a href="explore-centres.html" class="btn btn-primary">Back to centers</a></div>' + '</div>';
      renderTerminal(html, {
        allDone: true
      });
      return;
    ...
    var fails = {
      ...
      reverted: {
        ...
        x: 'Unable to complete this donation — the transaction was reverted and no funds reached the center. Please try again.',
        notify: true
      },
      rejected: {
    ...
    html = terminalShell(kind === 'rejected' ? 'pending' : 'fail', f.t, f.x + (f.notify ? ' If this keeps happening, contact support.' : ''), detailRow('Donation ID', donationId) + detailRow('Status', D.statusChip(kind === 'expired' ? 'expired' : kind === 'rejected' ? 'pending_payment' : 'failed')) + detailRow(multi ? 'Humanity Centers' : 'Humanity Center', centreLabel), '<a href="explore-centres.html" class="btn btn-secondary">Back to centers</a><button type="button" class="btn btn-primary" onclick="location.reload()">Try again</button>');
    renderTerminal(html, {
      fail: true
    });
  }
