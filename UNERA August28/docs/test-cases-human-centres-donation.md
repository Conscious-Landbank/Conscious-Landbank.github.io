# Test cases: Huma Humanity Centers and Donation

**Source of truth:** Confluence: *Huma - Humanity Centers & Donation (In-Review)* (page 74579981, v8, last modified Jul 08 2026)
**Scope:** Public HC browsing, HC detail, fiat & crypto donation, donation history & receipts, Huma-Points-from-donations, notifications, plus cross-cutting states, error handling, security, a11y, and responsive.
...
| Token | Meaning |
| --- | --- |
| `HC_ACTIVE` | Active Humanity Center, `is_active = true` |
...
| TC-DASH-04 | Happy | P2 | `USER_KYC`, Huma Points reward data present | Open page | "Huma Points earneds" card shows cumulative aggregate | DON-DASH-04, DON-UYT-04 |
...
## Suite 2: Humanity Center Directory (§3.2)
...
| TC-FIAT-06 | Happy | P2 | Completed + eligible | After completion | Huma Points earned event/calc triggered; shown as estimated/pending | DON-FIAT-09, DON-UYT-03 |
...
| TC-CRY-06 | Happy | P1 | Completed | Success screen | Shows txHash + explorer link, receipt status (if eligible), Huma Points earned state | DON-CRYPTO-07, §5.3 step 8, AC-DON-05 |
| TC-CRY-07 | Happy | P2 | Completed + eligible | After finality | Receipt uses **confirmed USD value**; Huma Points uses USD donation value | DON-CRYPTO-08/09 |
...
## Suite 7: Huma Points Gains from Donations (§3.7)

| ID | Type | Pri | Precondition | Steps | Expected | Trace |
| --- | --- | --- | --- | --- | --- | --- |
| TC-UYT-01 | Happy | P2 | Donation earns Huma Points, confirmed | Success screen | Shows confirmed Huma Points earned with clear label | DON-UYT-01/03, AC-DON-09 |
...
| TC-UYT-03 | Happy | P2 | Cumulative data available | Donation page | Cumulative Huma-Points-from-donations aggregate shown | DON-UYT-04 |
| TC-UYT-04 | Edge | P2 | Huma Points delayed/unavailable | After completion | Donation stays **completed**; Huma Points shows pending/unavailable independently | §7.2.2 step 7 |
| TC-UYT-05 | Edge | P3 | Huma Points status `failed` | View | Handled label; donation success unaffected | §7.1.5 status enum |
...
| TC-UYT-07 | Edge | P3 | FE display | Inspect values | Huma Points values come from Huma Points service; **not** hardcoded/calculated in FE | DON-UYT-02, Risk §9.1 |

## Suite 8: Notification (§4.3.1), the primary requested anchor
...
| TC-NOT-07 | Happy | P1 | Huma Points earning record confirmed | "Huma Points earned": **completed** | In-app / notification center | §4.3.1, DON-UYT-01 |
...
| TC-SEC-03 | Security | P1 | History/receipt/Huma Points endpoints | Require auth; user-scoped only | §4.2 |
...
| TC-RESP-04 | UI | P2 | Copy consistency | "Donation", "Humanity Center", "total donated", "lives impacted", "Huma Points earned" used consistently | §6.4 |
...
- Suite 4 (Fiat): review shows **0 wallet confirmations**, "Huma fee: None", no rate-lock. Confirm -> full lifecycle fires the exact §4.3.1 sequence: `Donation submitted` (info) -> `Payment instructions ready` (info) -> `Donation awaiting confirmation` (progressing) -> `Donation completed` (completed) -> `Tax receipt available` (info) -> `Huma Points earned` (completed). Terminal = success receipt. Below-min amount blocks with the §6.3 range message. -> TC-FIAT-01/02/03/08, TC-NOT-01->07.
...
- Suite 2/3 (Directory/Detail): directory shows only the 5 active centers (1 inactive hidden); search present. Inactive center -> Donate CTA **disabled** ("Donations unavailable"), generic paused banner, no internal reason leaked. -> TC-DIR-01/08, TC-DET-06.
...
- Live financial summary announces. The amount step's live "Center receives" summary (`#amountLive`) is `role="status" aria-live="polite"` so the USD outcome is announced as the donor types. -> TC-A11Y-06.
...
- Per-centre amounts on the success receipt. The multi-centre terminal now itemizes each center with its allocation and USD value (e.g. "Kibera Community Center · 40 USDC · ≈ $40.00"), matching §5.5 step 5 ("confirm each donation allocation") and the Fundraise Up/WinRed pattern of confirming every split. Review step already itemized; the receipt now matches. -> closes TC-CRY-27/28.
- Selection cap. Split is capped at **5 centers** (industry norm) with an inline `role="alert"` hint when exceeded; defensive today since only 5 active centers exist. -> TC-CRY-29.
...
# Suite 13: FE-208 additions (Confluence 21 Aug 2026 revision)

**Why this suite exists.** The 21 Aug revisions of *Huma Platform – Humanity Centers* (88768526) and *Huma Platform – Donation* (74579981) introduced the processing-fee model, the $1 minimum, the USDC/USDT-vs-convertible settlement split, the Uniswap route comparison, `conversion_pending`, the HC status enum and the "Donate by Fiat / Donate by Crypto" naming. Suites 1–12 predate all of that. Gap analysis: `docs/FE-208-audit.md`.

**New test data conventions**

| Token | Meaning |
| --- | --- |
| `HC_SUSPENDED` | HC with `status = suspended` (seed: `hc-dhaka`) |
| `HC_NO_LIVES` | Active HC with `lives_impacted = null` (seed: `hc-hanoi`) |
| `ASSET_DIRECT` | USDC or USDT: settles to the multisig with no swap |
| `ASSET_CONVERT` | BTC or ETH: converted to USDC/USDT by the Crypto Swap Worker |
| `CCY_NON_USD` | EUR / GBP / VND fiat donation |

## 13.1: Processing fee and total charged (DON-DASH-08, AC-DON-07)

| ID | Type | Pri | Precondition | Steps | Expected | Trace |
| --- | --- | --- | --- | --- | --- | --- |
| TC-FEE-01 | Happy | P1 | `USER_KYC`, fiat, $100 USD | Reach review | Costs group shows **Donation amount $100.00**, **Processing fee + $3.20** ("Applied on top of your donation"), **Total charged $103.20** | DON-DASH-08 |
| TC-FEE-02 | Happy | P1 | `USER_KYC_WALLET`, 150 USDC | Reach review | Processing fee is a separate `+` row; total charged = donation + fee; **network fee is excluded from total charged** and the row note says so | DON-CRYPTO-03 |
| TC-FEE-03 | Happy | P1 | `USER_KYC_WALLET`, 0.003 BTC | Reach review | A **Conversion fee** row appears between processing fee and network fee, labelled `BTC -> USDC` (or USDT) | DON-CRYPTO-03, DON-CRYPTO-05 |
| TC-FEE-04 | Edge | P2 | `USER_KYC_WALLET`, 150 USDC | Reach review | **No** conversion fee row: USDC needs no swap | DON-CRYPTO-04 |
| TC-FEE-05 | Happy | P2 | Any donation | Reach amount step, type an amount | The live summary already shows donation amount, processing fee and total charged **before** review | DON-DASH-08 |
| TC-FEE-06 | Happy | P2 | Any donation | Complete a donation | The success receipt itemises donation amount, processing fee, conversion fee (if any) and total charged | DON-DASH-08 |
| TC-FEE-07 | Happy | P2 | Any historic donation | Open history detail | Detail shows processing fee, conversion fee where applicable, and total charged | DON-HIST-04 |
| TC-FEE-08 | Edge | P3 | Any review screen | Read the costs group | A visible note flags the rates as prototype placeholders pending Finance | audit §14 |

## 13.2: $1 minimum (DON-DASH-09, AC-DON-08)

| ID | Type | Pri | Precondition | Steps | Expected | Trace |
| --- | --- | --- | --- | --- | --- | --- |
| TC-MIN-01 | Validation | P1 | `USER_KYC`, fiat | Enter `0.50`, press Review | Submission blocked; message is **exactly** "Donation amount must be between $1 and $50,000." | §6.3, AC-DON-08 |
| TC-MIN-02 | Happy | P1 | `USER_KYC`, fiat | Enter `1.00`, press Review | Accepted: $1 is valid, not rejected | DON-DASH-09 |
| TC-MIN-03 | Validation | P1 | `USER_KYC_WALLET`, ETH | Enter an ETH amount worth < $1 | Blocked with the same string; the USD *equivalent* is what is validated | DON-DASH-09 |
| TC-MIN-04 | Validation | P2 | Any method | Enter `abc` / empty / `0` | "Enter a valid donation amount." verbatim | §6.3 |
| TC-MIN-05 | Edge | P3 | Any method | Read the amount hint before typing | Hint reads "$1 and $50,000", never "$5" | DON-DASH-09 |

## 13.3: Method naming and routing (§5.1 step 3, AC-DON-02)

| ID | Type | Pri | Precondition | Steps | Expected | Trace |
| --- | --- | --- | --- | --- | --- | --- |
| TC-METH-01 | Happy | P1 | `USER_KYC` | Open step 2 | Exactly two options, labelled **"Donate by Fiat"** and **"Donate by Crypto"** | §5.1, AC-DON-02 |
| TC-METH-02 | A11y | P2 | `USER_KYC` | Tab to the selector | `role="radiogroup"` with two `role="radio"` children; arrow/space operable; `aria-checked` tracks selection | §6.4 |
| TC-METH-03 | Happy | P2 | `USER_KYC` | Read each option's subtitle | Fiat mentions no wallet needed; Crypto states USDC/USDT direct and BTC/ETH converted | §1.7 |

## 13.4: Fiat path, non-USD currencies (DON-FIAT-04, AC-DON-04)

| ID | Type | Pri | Precondition | Steps | Expected | Trace |
| --- | --- | --- | --- | --- | --- | --- |
| TC-FIAT-10 | Happy | P1 | `USER_KYC`, `CCY_NON_USD` = EUR | Select EUR, enter `100`, review | Review shows original **€100.00 EUR**, the **exchange rate**, the **USD value**, and a USD total charged plus the amount charged in EUR | DON-FIAT-04, AC-DON-04 |
| TC-FIAT-11 | Happy | P2 | EUR donation completes | Open history detail | Original currency, original amount and rate are preserved alongside the USD value | DON-FIAT-04 |
| TC-FIAT-12 | Happy | P1 | USD donation | Reach review | Destination reads **"Payment Rail settlement account"** with the note "Fiat is not converted to crypto" | DON-FIAT-03 |
| TC-FIAT-13 | Edge | P3 | Any fiat donation | Read the currency selector | The helper line reads as product copy ("More currencies are on the way…"), no spec jargon; the OQ-02 open question is tracked in FE-208-audit.md, not in UI | §9.2 OQ-02 |
| TC-FIAT-14 | Edge | P2 | `USER_KYC`, no saved card | Open step 2 with fiat | "No saved cards yet" hint plus an Add-card affordance; Review is blocked with a clear message | DON-FIAT-01 |

## 13.5: crypto path with settlement and Uniswap routing (DON-CRYPTO-01…06)

| ID | Type | Pri | Precondition | Steps | Expected | Trace |
| --- | --- | --- | --- | --- | --- | --- |
| TC-CRY-30 | Happy | P1 | `USER_KYC_WALLET` | Open the asset selector | **USDC and USDT appear first**, tagged "Direct"; BTC and ETH follow, tagged "Converted to USDC/USDT" | DON-CRYPTO-01 |
| TC-CRY-31 | Happy | P1 | `ASSET_DIRECT`, amount entered | Read the quote line | Shows the USD value, attributes it to **Uniswap**, and states it is the **best of the USDC and USDT routes** | DON-CRYPTO-02 |
| TC-CRY-32 | Happy | P1 | `ASSET_CONVERT` = BTC | Reach review | **Settlement asset** row names USDC or USDT (whichever route wins); a conversion note explains the swap | DON-CRYPTO-05, AC-DON-06 |
| TC-CRY-33 | Happy | P1 | Any crypto donation | Reach review | **Destination type** = "Humanity Center multisig wallet" and the **wallet address** is shown | DON-CRYPTO-03 |
| TC-CRY-34 | Edge | P2 | Any crypto donation | Reach review | The network row carries a visible **TBD** for the approved-network list (OQ-03) | §9.2 OQ-03 |
| TC-CRY-35 | Happy | P1 | `ASSET_CONVERT`, confirm | Watch the Transaction Tracker | Stage list includes **"Converting to USDC"** (or USDT) between confirmation and delivery | DON-CRYPTO-06, DON-CRYPTO-07 |
| TC-CRY-36 | Happy | P1 | `ASSET_DIRECT`, confirm | Watch the tracker | **No** conversion stage appears: the stage list is one shorter | DON-CRYPTO-04 |
| TC-CRY-37 | Happy | P1 | Any crypto donation, confirm | Watch the tracker | First stage is **"Waiting for your signature"**, then **"Submitted to the network"**, then **"Awaiting confirmation"** | DON-CRYPTO-07 |
| TC-CRY-38 | Edge | P1 | Outcome pill = "Conversion pending" | Confirm and wait | Terminal shows status chip **"Conversion pending"**, the verbatim message "Your donation is being processed. We'll update the status when conversion is complete.", and **is never labelled Completed** | DON-CRYPTO-07, §6.3 |
| TC-CRY-39 | Happy | P2 | Successful crypto donation | Read the success receipt | txHash with an explorer link, settlement asset, destination type, receipt status and Huma Points state all present | §5.3 step 10 |
| TC-CRY-40 | Edge | P2 | `USER_KYC_WALLET` | Read the asset list | **hUSD is absent** pending confirmation that it is a valid settlement asset (§1.7 lists only USD_FIAT/USDC/USDT) | audit §14 |

## 13.6: HC status enum and directory hygiene (HC-DIR-01, HC-DETAIL-06, HC-CRUD-03)

| ID | Type | Pri | Precondition | Steps | Expected | Trace |
| --- | --- | --- | --- | --- | --- | --- |
| TC-DIR-20 | Happy | P1 | `HC_SUSPENDED` exists | Open the directory | The suspended HC **does not appear**, and there is **no filter that can reveal it** | HC-DIR-01 |
| TC-DIR-21 | Happy | P1 | Active HCs with categories | Open the category filter | Options are **derived from the HC records**, not hard-coded, and filter correctly | HC-DIR-03 |
| TC-DIR-22 | Happy | P2 | Any HC card | Read the card | Shows name, hero image, **city + country**, category chip, total donated and lives impacted | HC-DIR-02 |
| TC-DIR-23 | Edge | P1 | `HC_NO_LIVES` | Find that card | Lives impacted reads **"Coming soon"**, styled as a state, never `0` | HC-DIR-02, AC-HC-04 |
| TC-DIR-24 | Edge | P2 | Aggregate hero | Read the footnote | A visible **TBD** covers whether anonymous on-chain donations are in public totals (OQ-01) | §9.2 OQ-01 |
| TC-DET-20 | Edge | P1 | `HC_SUSPENDED` | Open `centre-detail.html?hc=hc-dhaka` directly | Page renders; status badge reads **"Suspended"**; Donate CTA is **disabled** with `aria-disabled="true"`; a safe explanatory banner names the status | HC-DETAIL-06, AC-HC-03 |
| TC-DET-21 | Edge | P1 | `HC_SUSPENDED` | Open `donate.html?hc=hc-dhaka` | The HC is **not preselected**; a safe banner names it and asks the user to choose an active center; the picker lists active HCs only | AC-DON-12 |
| TC-DET-22 | Edge | P2 | Any HC, metrics unavailable pill | Toggle the pill | Both metric cards read **"Metrics unavailable"**; the profile, the programs and the gallery still render | HC-DETAIL-03, §6.2 |
| TC-DET-23 | Happy | P2 | Any HC | Read the location line | Shows **city, country** and the public category chip | HC-DETAIL-02 |
| TC-DET-24 | Security | P1 | Any HC detail | Inspect the page | No internal governance, compliance or operational notes; no donor PII | HC-DETAIL-07, AC-HC-08 |

## 13.7: Login return path (flow 5.1 steps 7–8)

| ID | Type | Pri | Precondition | Steps | Expected | Trace |
| --- | --- | --- | --- | --- | --- | --- |
| TC-AUTH-10 | Happy | P1 | `USER_PUBLIC` | Click Donate on a directory card | Login prompt names the center; the Log-in link carries `?next=donate.html%3Fhc%3D<id>` | §5.1 steps 7–8 |
| TC-AUTH-11 | Happy | P1 | `USER_PUBLIC` | Click Donate on the HC detail page | Same: the return URL preserves the HC | HC-DETAIL-05, §5.1 |
| TC-AUTH-12 | Happy | P2 | Return from login | Land on donate | Step 1 opens with the HC **already selected** | §5.1 step 8 |

## 13.8: Donation page: value proposition and streams

| ID | Type | Pri | Precondition | Steps | Expected | Trace |
| --- | --- | --- | --- | --- | --- | --- |
| TC-VP-01 | Happy | P2 | Any user | Open the Donation page | Hero states the positioning line and shows six value items: auditable, $1 minimum, hUSD 1:1, receipts tied to the transaction ID, no bank required, dual funding | Value Proposition |
| TC-VP-02 | Happy | P2 | Any user | Scroll to "How your giving multiplies" | All **six** streams present and correctly named, each with its Huma Points multiplier (or an explicit "no points" note for Passive Reserve Yield and Corporate Match) | Value Proposition |
| TC-VP-03 | Happy | P3 | Any user | Read the allocation footnote | States Impact Points voting and that final authority is the Swiss Association | Value Proposition |
| TC-VP-04 | Security | P2 | `USER_PUBLIC` | Open the Donation page | Value proposition and streams are visible **without login**; personal summary cards are not | §6.2 Unauthenticated |

## 13.9: Huma Points display states (DON-HP-01…03)

| ID | Type | Pri | Precondition | Steps | Expected | Trace |
| --- | --- | --- | --- | --- | --- | --- |
| TC-HP-10 | Happy | P2 | Data-state pill = estimated | Open the Donation page | Card badge reads **Estimated** with an explanatory sub-line | DON-HP-02 |
| TC-HP-11 | Happy | P2 | pill = pending | Same | Badge reads **Pending** | DON-HP-02 |
| TC-HP-12 | Happy | P2 | pill = confirmed (default) | Same | Badge reads **Confirmed** | DON-HP-02 |
| TC-HP-13 | Edge | P2 | pill = unavailable | Same | Value reads "Unavailable" with the **Unavailable** badge, never `0` points | §4.1 |
| TC-HP-14 | Happy | P1 | Successful donation | Read the success screen | Points shown with the **Estimated** state label, not as a confirmed figure | DON-HP-02 |
| TC-HP-15 | Security | P1 | Any surface | Inspect the copy | The UI states no multiplier/tier/fee-discount rule as fact beyond the informational stream table, which is labelled as display-only | DON-HP-03 |

## 13.10: History rows and detail (DON-HIST-02, DON-HIST-04)

| ID | Type | Pri | Precondition | Steps | Expected | Trace |
| --- | --- | --- | --- | --- | --- | --- |
| TC-HIST-20 | Happy | P1 | Crypto donation in history | Read the row | Shows HC, original amount + currency, USD equivalent, source, status, date, **settlement asset**, txHash and explorer link | DON-HIST-02 |
| TC-HIST-21 | Happy | P1 | Non-USD fiat donation | Read the row | Secondary amount shows the **original currency amount**, primary shows USD | DON-HIST-02 |
| TC-HIST-22 | Happy | P1 | Any donation | Open the detail view | Donation ID, HC, status, original amount, USD value, **processing fee**, conversion fee where applicable, total charged, settlement asset, **destination type**, receipt and txHash all present | DON-HIST-04 |
| TC-HIST-23 | Edge | P1 | `conversion_pending` row | Open the detail | Status chip reads **Conversion pending**; settlement amount reads "Not settled yet"; receipt is pending | DON-CRYPTO-07 |
| TC-HIST-24 | Edge | P2 | Receipt `unavailable` row | Open the detail | Receipt row reads **"A receipt is not available for this donation."** verbatim; no download button | §6.3, DON-HIST-05 |
| TC-HIST-25 | Happy | P2 | Receipt `available` row | Open the detail | Receipt number shown with the note that it is tied to the transaction ID; download button present | DON-HIST-05 |
| TC-HIST-26 | Happy | P2 | Mixed history | Filter by status "Pending / awaiting confirmation / conversion" | `awaiting_confirmation` **and** `conversion_pending` rows both match | DON-HIST-03 |

## 13.11: Notifications (§4.3)

| ID | Type | Pri | Precondition | Steps | Expected | Trace |
| --- | --- | --- | --- | --- | --- | --- |
| TC-NOT-20 | Happy | P1 | Cold load, empty feed | Open the bell | All six donation events are seeded: submitted, awaiting confirmation, completed, failed, receipt available, Huma Points updated | §4.3 |
| TC-NOT-21 | Happy | P2 | Existing stored feed from an earlier build | Reload | The six new seeds merge in **without** resetting read state on existing items | §4.3 |
| TC-NOT-22 | Happy | P1 | Confirm a donation | Watch the bell | "Donation submitted" fires immediately, "Donation awaiting confirmation" follows | §4.3 |
| TC-NOT-23 | Happy | P1 | Donation completes | Watch the bell | "Donation completed" -> "Tax receipt available" -> "Huma Points updated", in that order | §4.3 |
| TC-NOT-24 | Error | P2 | Donation fails | Watch the bell | "Donation failed" carries the verbatim §6.3 reason and a Try-again CTA | §4.3 |
| TC-NOT-25 | Edge | P3 | Any seeded donation notification | Read the copy | No retired tokens (CTC/UYT) and no centres absent from the HC records | §6.4 |

## 13.12: §6.2 states not previously covered

| ID | Type | Pri | Precondition | Steps | Expected | Trace |
| --- | --- | --- | --- | --- | --- | --- |
| TC-STATE-20 | Edge | P2 | Blocker pill = "Service unavailable" | Open the donate flow | A neutral "Donations are temporarily unavailable" banner appears and Review is disabled | §6.2 Unavailable |
| TC-STATE-21 | Edge | P2 | Data-state pill = Loading | Open the Donation page | Skeletons in the summary cards, recent list and balance card; no `0`/`-` flash | §6.2 Loading |
| TC-STATE-22 | Edge | P2 | Blocker pill = "Center becomes inactive" | Sit on review | Blocking banner appears and Confirm is disabled | AC-DON-12 |

## 13.13: Scope conflict to re-test once Product decides

| ID | Type | Pri | Note |
| --- | --- | --- | --- |
| TC-SCOPE-01 | Edge | P1 | Donation §1.4 now lists multi-HC allocation as **out of scope** while TC-CRY-27->29 still test it. The split UI is currently hidden unless `?multi=1`. **Re-test or retire TC-CRY-27->29 once Product rules.** See `docs/FE-208-audit.md` §14. |
