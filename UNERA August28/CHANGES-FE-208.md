# FE-208 · Humanity Centers and Donation design update
...
**New copy rules** are captured in full in `docs/UNERA-terminology-and-copy.md §10`. The two that will bite a future edit:
...
- Zero genuine console errors. The console log is non-empty only because the sandbox deliberately aborts outbound requests (no egress in this environment), so the Unsplash card photos report `net::ERR_FAILED`. Those are the seed data's external image URLs, unchanged by this ticket. Nothing else errors.
