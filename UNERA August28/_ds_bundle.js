...
(function () {
    ...
    return {
      ...
      interactive: {
        ...
        frames: crypto ? [{
          ...
          body: 'A background worker converts your ' + token + ' at whichever Uniswap route returns more USD value. Automatic.'
        }] : []).concat([{
          art: TRK_ART.heart,
  ...
  renderTokens();
  renderAmountUI();
  /* Slack feedback (Kevin, 28 Aug) - the dashboard shortcut passes ?amt=; skip step 1 and land on the
     amount step with it prefilled (USD, fiat default). Out-of-range values fall back to the normal flow. */
  var preAmt = parseFloat(D.qs('amt'));
  if (selected.length && !preBlocked && !isNaN(preAmt) && preAmt >= MIN_USD && preAmt <= MAX_USD) {
    $('amountInput').value = preAmt;
    updateAmountLive();
    goToStep(2);
  }
  if (preBlocked) {
